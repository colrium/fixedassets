  ...
  alert('File picker requires jquery');
	...
	$.widget("collinslibs.filepicker", {
			...
			mimes: {
				...
				sound : ["zip", "tar", "gzip"],
				video : ["mp4", "avi", "mov", "mpeg", "flv", "3gp"]

			},
			template : "<div class=\"fileuploader-input\">" +
								"<div class=\"fileuploader-input-inner center-align\">" +
									"<h1 class=\"full-width center-align\"><i class=\"material-icons medium cfpicker-icon\">{icon}</i></h1>" +
									"<h3 class=\"fileuploader-input-caption full-width center-align\">{caption}<span></span></h3>" +
									"<p class=\"full-width center-align\">or</p>" +
									"<div class=\"fileuploader-input-button waves-effect waves-light\"><span>{browsebtntext}</span></div>" +
								"</div>" +
						"</div>",
		    DEFAULTS: {
		    	theme: "dragdrop",
		    	...
		    	removeurl: "#",		    	
				icon: "attach_file",
				caption: "Drag and drop file here",
				dropcaption: "Drag and drop file here",
				pastecaption: "<div class=\"fileuploader-pending-loader\"><div class=\"left-half\" style=\"animation-duration: ${ms}s\"></div><div class=\"spinner\" style=\"animation-duration: ${ms}s\"></div><div class=\"right-half\" style=\"animation-duration: ${ms}s\"></div></div> Pasting a file, click here to cancel.",
			...
			_build: function(){
				...
				var mime = instance.DEFAULTS.filestype;
				
				if (mime != null) {
					if(!$.isArray(mime)){
						if ($.contains(instance.mimes, mime)) {
							instance.extensions = instance.mimes[mime];
						}
					}
					else{
						instance.extensions = mime;
					}

						
				}
				//build layout
				instance.template = instance.template.replace("{icon}", instance.DEFAULTS.icon);
				instance.template = instance.template.replace("{caption}", instance.DEFAULTS.caption);
				instance.template = instance.template.replace("{browsebtntext}", instance.DEFAULTS.browsebtntext);

				//bind progress resume event
				instance.element.bind('onsuccess', function(event, data, item){
					
					if (data != null) {
						if(data.erroroccured == 0) {
							item.name = data.filesdetails.name;
							item.databaseid = data.filesdetails.databaseid;							
						}					        
						item.html.find(".column-actions").append("<a class=\"fileuploader-action fileuploader-action-remove fileuploader-action-success\" title=\"Remove\"><i class=\"material-icons spaced-text\">delete</i></a>");
						setTimeout(function() {
							item.html.find(".progress-bar2").fadeOut(400);
						}, 400);
					}	
						
					if(jQuery.isFunction(instance.DEFAULTS.onSuccess)){
						instance.DEFAULTS.onSuccess(event, data);
					}
				});
			...
			_initialize: function(){
				var instance = this;
				this.element.fileuploader({
					changeInput: instance.template,
					theme: instance.DEFAULTS.theme,
					extensions: instance.extensions,
					limit: instance.DEFAULTS.maxcount,
					maxSize: instance.DEFAULTS.totalmaxsize,
					fileMaxSize: instance.DEFAULTS.singlemaxsize,
					addMore: false,
					upload: {
					    url: instance.DEFAULTS.uploadurl,
					    data: null,
					    type: "POST",
					    ...
					    onSuccess: function(result, item) {
					    	...
					    	try{
					    		...
					    		$(instance.element).trigger( "onsuccess", [data, item]);
					    	}
					    	catch(exception){
					    		console.log(exception);
					    		$(instance.element).trigger( "onerror", [data, item]);
					    	}
					...
					onRemove: function(item) {
						$.post(instance.DEFAULTS.removeurl+"/"+item.databaseid, {
							file: item.name
						});
					},
					captions: {

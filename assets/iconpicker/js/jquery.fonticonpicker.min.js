/**
 *  jQuery materialIconPicker - v2.0.0
 *
 *  An icon picker built on top of font icons and jQuery
 *
 *  http://codeb.it/materialIconPicker
 *
 *  Made by Alessandro Benoit & Swashata
 *  Under MIT License
 *
 * {
@link https://github.com/micc83/materialIconPicker}
 */
!function(a){
"use strict";
function c(c,d){

	this.element=a(c),this.settings=a.extend({
	},b,d),
	this.settings.emptyIcon&&this.settings.iconsPerPage--,
	this.iconPicker = a("<div/>",{
		"class":"icons-selector",
		style:"position: relative",
		html:'<div class="selector"><span class="selected-icon"><i class="material-icons spaced-text">block</i></span><span class="selector-button"><i class="material-icons spaced-text">keyboard_arrow_down</i></span></div><div class="selector-popup" style="display: none;">'+(this.settings.hasSearch?'<div class="row"><div class="input-field col s12"><input type="text" name="" value="" id="iconpickersearchinput"/><label for="iconpickersearchinput"><i class="material-icons spaced-text">search</i> Search Icon</label></div></div>':"")+'<div class="row">'+'<div class="input-field col s12"><select name="" class="icon-category-select">'+"</select></div>"+"</div>"+'<div class="fip-icons-container"></div>'+'<div class="selector-footer" style="display:none;">'+'<span class="selector-pages">1/2</span>'+'<span class="selector-arrows">'+'<span class="selector-arrow-left" style="display:none;">'+'<i class="material-icons spaced-text">keyboard_arrow_left</i>'+"</span>"+'<span class="selector-arrow-right">'+'<i class="material-icons spaced-text">keyboard_arrow_right</i>'+"</span>"+"</span>"+"</div>"+"</div>"
	}),
	...
	this.selectedCategory=!1,this.availableCategories=[],
	this.unCategorizedKey=null,this.init()
}
	var b={
		theme:"fip-grey",
		source:!1,
...
left:""}),this.element.before(this.iconPicker),this.element.css({
visibility:"hidden",top:0,position:"relative",zIndex:"-1",left:"-"+c+"px",display:"inline-block",height:b+"px",width:c+"px",padding:"0",margin:"0 -"+c+"px 0 0",border:"0 none",verticalAlign:"top"}),!this.element.is("select")){
var d=function(){
for(var a=3,b=document.createElement("div"),c=b.all||[];
b.innerHTML="<!--[if gt IE "+ ++a+"]><br><![endif]-->",c[0];
);
return a>4?a:!a}(),e=document.createElement("div");
this.triggerEvent=9!==d&&"oninput"in e?["input","keyup"]:["keyup"]}!this.settings.source&&this.element.is("select")?(this.settings.source=[],this.settings.searchSource=[],this.element.find("optgroup").length?(this.isCategorized=!0,this.element.find("optgroup").each(a.proxy(function(b,c){
...
var e=parseInt(d,10);
this.availableCategories[e]&&(this.settings.source=this.availableCategories[e],this.searchValues=this.availableCategoriesSearch[e])}this.resetSearch(),this.loadIcons()},
...
return a.stopPropagation(),!1}),a("html").click(a.proxy(function(){
...
return"function"==typeof a.toString?a.toString():a}),this.searchValues=a.isArray(this.settings.searchSource)?a.map(this.settings.searchSource,function(a){
return"function"==typeof a.toString?a.toString():a}):this.settings.source.slice(0);
else{
var b=a.extend(!0,{
},
...
"function"==typeof g.toString&&(g=g.toString()),g&&g!==this.settings.emptyIconValue&&(this.settings.source.push(g),this.availableCategories[d].push(g),this.searchValues.push(h),this.availableCategoriesSearch[d].push(h))}}}this.backupSource=this.settings.source.slice(0),this.backupSearch=this.searchValues.slice(0),this.loadCategories()}},
...
loadIcons:function(){
this.iconContainer.html('<i class="material-icons  spaced-text animate-spin loading">hourglass_full</i>'),this.settings.source instanceof Array&&this.renderIconContainer()},
renderIconContainer:function(){
var b,c=[];
if(c=this.isSearch?this.iconsSearched:this.settings.source,this.iconsCount=c.length,this.totalPage=Math.ceil(this.iconsCount/this.settings.iconsPerPage),this.totalPage>1?this.iconPicker.find(".selector-footer").show():this.iconPicker.find(".selector-footer").hide(),this.iconPicker.find(".selector-pages").html(this.currentPage+"/"+this.totalPage+" <em>("+this.iconsCount+")</em>"),b=(this.currentPage-1)*this.settings.iconsPerPage,this.settings.emptyIcon)this.iconContainer.html('<span class="fip-box"><i class="material-icons spaced-text" data-fip-value="block">block</i></span>');
else{
if(c.length<1)return this.iconContainer.html('<span class="icons-picker-error"><i class="material-icons spaced-text" data-fip-value="block">block</i></span>'),void 0;
this.iconContainer.html("")}c=c.slice(b,b+this.settings.iconsPerPage);
for(var e,d=0;
e=c[d++];
){
var f=e;
...
this)),a("<span/>",{
html:'<i data-fip-value="'+e+'" '+(this.settings.useAttribute?this.settings.attributeName+'="'+(this.settings.convertToHex?"&#x"+parseInt(e,10).toString(16)+";":e)+'"':'class="material-icons"')+">"+e+"</i>","class":"fip-box",title:f}).appendTo(this.iconContainer)}this.settings.emptyIcon||this.element.val()&&-1!==a.inArray(this.element.val(),this.settings.source)?-1===a.inArray(this.element.val(),this.settings.source)?this.setSelectedIcon():this.setSelectedIcon(this.element.val()):this.setSelectedIcon(c[0])},
...
if("fip-icon-block"===a&&(a=""),this.settings.useAttribute?a?this.iconPicker.find(".selected-icon").html("<i "+this.settings.attributeName+'="'+(this.settings.convertToHex?"&#x"+parseInt(a,10).toString(16)+";":a)+'"></i>'):this.iconPicker.find(".selected-icon").html('<i class="material-icons">block</i>'):this.iconPicker.find(".selected-icon").html('<i class="material-icons spaced-text">'+(a||"block")+'</i>'),this.element.val(""===a?this.settings.emptyIconValue:a).trigger("change"),null!==this.triggerEvent)for(var b in this.triggerEvent)this.element.trigger(this.triggerEvent[b]);
this.currentIcon=a,this.setHighlightedIcon()},
toggleIconSelector:function(){
this.open=this.open?0:1,this.iconPicker.find(".selector-popup").slideToggle(300),this.iconPicker.find(".selector-button i").text("keyboard_arrow_down"),this.iconPicker.find(".selector-button i").text("keyboard_arrow_up"),this.open&&this.iconPicker.find(".icons-search-input").focus().select()},
...
a.fn.materialIconPicker=function(b){
return this.each(function(){
a.data(this,"materialIconPicker")||a.data(this,"materialIconPicker",new c(this,b))}),this.setIcons=a.proxy(function(b,c){
void 0===b&&(b=!1),void 0===c&&(c=!1),this.each(function(){
a.data(this,"materialIconPicker").settings.source=b,a.data(this,"materialIconPicker").settings.searchSource=c,a.data(this,"materialIconPicker").initSourceIndex(),a.data(this,"materialIconPicker").resetSearch(),a.data(this,"materialIconPicker").loadIcons()})},
this),this.destroyPicker=a.proxy(function(){
this.each(function(){
a.data(this,"materialIconPicker")&&(a.data(this,"materialIconPicker").iconPicker.remove(),a.data(this,"materialIconPicker").element.css({
visibility:"",top:"",position:"",zIndex:"",left:"",display:"",height:"",width:"",padding:"",margin:"",border:"",verticalAlign:""}),a.removeData(this,"materialIconPicker"))})},
this),this.refreshPicker=a.proxy(function(d){
d||(d=b),this.destroyPicker(),this.each(function(){
a.data(this,"materialIconPicker")||a.data(this,"materialIconPicker",new c(this,d))})},
this),this}}(jQuery);

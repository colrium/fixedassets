...
class Ion_auth_model extends CI_Model{
	...
	public $usergroupspriveledeges;

	public $userpriveledeges;
	...
	public function __construct() {
		...
		$this->usergroupspriveledeges = array();
		$this->userpriveledeges = array();
	...
	public function salt(){
		$salt = genrandomstrid($this->salt_length);
		return $salt;
	...
	public function add_to_group($group_ids, $user_id=false)
	{
		...
		{
			if ($this->db->insert($this->tables['users_groups'], array('id'=>genrandomstrid(11), $this->join['groups'] => $group_id, $this->join['users'] => $user_id)))
			{
	...
	public function remove_from_group($group_ids=false, $user_id=false)
	{
		...
		if( ! empty($group_ids))
		{
			...
			foreach($group_ids as $group_id)
			{
				$this->db->delete($this->tables['users_groups'], array($this->join['groups'] => $group_id, $this->join['users'] => $user_id));
	...
	**/
	...
	public function set_session($user){
		...
		$session_data = array(
			...
			'user_image' => dbhasmainimage('users', $user->id),
			'locked' 			   => 0
		);
	...
	public function login_remembered_user(){
		...
		$identity_cookie = get_cookie($identity_cookie_name);
		$remember_cookie = get_cookie($remember_cookie_name);
		$identity_header = $this->input->get_request_header($identity_cookie_name, TRUE);
		$remember_header = $this->input->get_request_header($remember_cookie_name, TRUE);
		$identity = $identity_cookie? $identity_cookie : $identity_header;
		$remember_code = $remember_cookie? $remember_cookie : $remember_header;


		// check for valid data
		if (!$identity || !$remember_cookie || !$this->identity_check($identity_cookie)){
	...
	**/
	...
	public function userpriveledges($userid=FALSE){
		$userid || $userid = $this->session->userdata('user_id');

		$priveledgesallocstable = $this->tables['priveledgeallocs'];
		$priveledgesdefstable = $this->tables['priveledges'];
		
		$usergroups = $this->get_users_groups($userid);
		
		$grpprivelegdes = array();
		$usrprivelegdes = array();

		$sqlStr ="SELECT 
							`$priveledgesallocstable`.`id` AS `id`,
							`$priveledgesallocstable`.`priveledgeid` AS `priveledgeid`,
							`$priveledgesallocstable`.`target` AS `target`,
							`$priveledgesallocstable`.`targetid` AS `targetid`,
							`$priveledgesallocstable`.`value` AS `value`,
							`$priveledgesdefstable`.`id` AS `defid`,
							`$priveledgesdefstable`.`name` AS `defname`,
							`$priveledgesdefstable`.`identityname` AS `defidentityname`,
							`$priveledgesdefstable`.`type` AS `deftype`,
							`$priveledgesdefstable`.`entity` AS `defentity`,
							`$priveledgesdefstable`.`includeadmins` AS `defincludeadmins`,
							`$priveledgesdefstable`.`datatable` AS `defdatatable`,
							`$priveledgesdefstable`.`datacolumn` AS `defdatacolumn`

								FROM 
									`$priveledgesallocstable` AS `$priveledgesallocstable`,
									`$priveledgesdefstable` AS `$priveledgesdefstable` 

									WHERE `$priveledgesdefstable`.`id` = `$priveledgesallocstable`.`priveledgeid`
									AND `$priveledgesallocstable`.`target` = 'user'
									AND `$priveledgesallocstable`.`targetid` = '$userid';";
				$query = $this->db->query($sqlStr);
				$userpriveledges = $query->result();
				foreach ($userpriveledges as $userpriveledge) {					
					if (!array_key_exists($userpriveledge->defentity, $usrprivelegdes)) {
						$usrprivelegdes[$userpriveledge->defentity] = array();
					}
					if (!array_key_exists($userpriveledge->defname, $usrprivelegdes[$userpriveledge->defentity])) {
						if($userpriveledge->deftype == 'access'){
							$usrprivelegdes[$userpriveledge->defentity][$userpriveledge->defname] = $userpriveledge->value;
						}
						else if($userpriveledge->deftype == 'data'){
							$usrprivelegdes[$userpriveledge->defentity][$userpriveledge->defname] = explode(',', $userpriveledge->datavalue);
						}												
					}
					else{
						if($userpriveledge->deftype == 'access'){
							if($usrprivelegdes[$userpriveledge->defentity][$userpriveledge->defname] == '0' && $userpriveledge->value == '1'){
								$usrprivelegdes[$userpriveledge->defentity][$userpriveledge->defname] = $userpriveledge->value;
							}							
						}
						else if($userpriveledge->deftype == 'data'){
							$newdatavalues = explode(',', $userpriveledge->datavalue);
							array_push($usrprivelegdes[$userpriveledge->defentity][$userpriveledge->defname], $newdatavalues);
						}											
					}
					
				}

		$this->userpriveledeges = $usrprivelegdes;

		if (is_array($usergroups) && sizeof($usergroups)>0) {
			...
			foreach ($usergroups as $key => $value) {
				$sqlStr ="SELECT 
							...
							`$priveledgesallocstable`.`targetid` AS `targetid`,
							`$priveledgesallocstable`.`value` AS `value`,
							`$priveledgesallocstable`.`datavalue` AS `datavalue`,
							`$priveledgesdefstable`.`id` AS `defid`,
							`$priveledgesdefstable`.`name` AS `defname`,
							`$priveledgesdefstable`.`identityname` AS `defidentityname`,
							`$priveledgesdefstable`.`type` AS `deftype`,
							...
							`$priveledgesdefstable`.`datacolumn` AS `defdatacolumn`
								...
								FROM 
									...
									WHERE `$priveledgesdefstable`.`id` = `$priveledgesallocstable`.`priveledgeid`
									AND `$priveledgesallocstable`.`target` = 'group'
									AND `$priveledgesallocstable`.`targetid` = '".$value->id."';";
				$query = $this->db->query($sqlStr);
				$grouppriveledges = $query->result();
				foreach ($grouppriveledges as $grouppriveledge) {					
					if (!array_key_exists($grouppriveledge->defentity, $grpprivelegdes)) {
						$grpprivelegdes[$grouppriveledge->defentity] = array();
					}
					if (!array_key_exists($grouppriveledge->defname, $grpprivelegdes[$grouppriveledge->defentity])) {
						if($grouppriveledge->deftype == 'access'){
							$grpprivelegdes[$grouppriveledge->defentity][$grouppriveledge->defname] = array('type' => $grouppriveledge->deftype, 'allowed' => $grouppriveledge->value, 'datatable' => $grouppriveledge->defdatatable, 'datacolumn' => $grouppriveledge->defdatacolumn, 'datavalue' => $grouppriveledge->datavalue, 'adminoverride' => $grouppriveledge->defincludeadmins);
						}
						else if($grouppriveledge->deftype == 'data'){
							$grpprivelegdes[$grouppriveledge->defentity][$grouppriveledge->defname] = array('type' => $grouppriveledge->deftype, 'allowed' => $grouppriveledge->value, 'datatable' => $grouppriveledge->defdatatable, 'datacolumn' => $grouppriveledge->defdatacolumn, 'datavalue' => explode(',', $grouppriveledge->datavalue), 'adminoverride' => $grouppriveledge->defincludeadmins);
						}												
					}
					else{
						if($grouppriveledge->deftype == 'access'){
							if($grpprivelegdes[$grouppriveledge->defentity][$grouppriveledge->defname]['allowed'] == '0' && $grouppriveledge->value == '1'){
								$grpprivelegdes[$grouppriveledge->defentity][$grouppriveledge->defname]['allowed'] = $grouppriveledge->value;
							}							
						}
						else if($grouppriveledge->deftype == 'data'){
							$newdatavalues = explode(',', $grouppriveledge->datavalue);
							array_push($grpprivelegdes[$grouppriveledge->defentity][$grouppriveledge->defname]['datavalue'], $newdatavalues);
						}											
					}
					
				}
			}
			$this->usergroupspriveledeges = $grpprivelegdes;
			$allpriveledges = array_merge($grpprivelegdes, $usrprivelegdes);
			return $allpriveledges;
	...
	public function haspriveledge($priveledge, $module, $userid, $data=FALSE){		
		$priveledged = FALSE;
		$admin_group = $this->config->item('admin_group', 'ion_auth');

		$is_admin = FALSE;

		if (array_key_exists($userid, $this->_cache_user_in_group)) {
			if (in_array($admin_group, $this->_cache_user_in_group[$userid])) {
				$is_admin = TRUE;
			}
		}
		...
		if(array_key_exists($module, $this->usergroupspriveledeges)){
			if (array_key_exists($priveledge, $this->usergroupspriveledeges[$module])) {
				if (is_array($this->usergroupspriveledeges[$module][$priveledge]['datavalue'])) {
					if ($data != FALSE && in_array($data, $this->usergroupspriveledeges[$module][$priveledge]['datavalue'])) {
						if ($this->usergroupspriveledeges[$module][$priveledge]['value'] == 1) {
							$priveledged = TRUE;
						}
						else{
							$priveledged = FALSE;
					...
					else if ($data != FALSE && !in_array($data, $this->usergroupspriveledeges[$module][$priveledge]['datavalue'])){
						if ($this->usergroupspriveledeges[$module][$priveledge]['value'] == 0) {
							$priveledged = TRUE;
						}
						
					}
				}
				else if ($this->usergroupspriveledeges[$module][$priveledge]['value'] == 1) {
					$priveledged = TRUE;
				}
				else{
					if ($is_admin == TRUE) {
		...
		if(array_key_exists($module, $this->userpriveledeges)){
			if (array_key_exists($priveledge, $this->userpriveledeges[$module])) {
				if (is_array($this->userpriveledeges[$module][$priveledge])) {
					if ($data != FALSE && in_array($data, $this->userpriveledeges[$module][$priveledge])) {
						$priveledged = TRUE;
					}
					else{
						$priveledged = FALSE;
				...
				else if ($this->userpriveledeges[$module][$priveledge] == 1) {
					$priveledged = TRUE;
				}
				else{
					if ($priveledged == TRUE) {
		...
		return $this->usergroupspriveledeges;
	}
}

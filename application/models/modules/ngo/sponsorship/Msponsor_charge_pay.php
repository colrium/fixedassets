...
class Msponsor_charge_pay extends CI_Model{
   ...
   //-----------------------------------------------------------------------
      ...
      $sqlStr =
         "SELECT
          ...
          FROM ngo_sponsor_charges
             INNER JOIN ngo_admin_aco              ON aco_lKeyID     = spc_lACOID
             LEFT  JOIN ngo_sponsor_autocharge_log ON spc_lAutoGenID = spcl_lKeyID
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
           "SELECT SUM(gi_curAmnt) AS curSum
            FROM ngo_gifts
               INNER JOIN ngo_sponsor ON gi_lSponsorID=sp_lKeyID
            WHERE
               gi_lACOID=$lACOID
               $strWhereDate
               $strWhereVia
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
           "CREATE TEMPORARY TABLE ngo_tmp_spon_balance (
      ...
      foreach($this->chargesTotal as $clsACOCharge){
         ...
         $sqlStr =
            'INSERT INTO ngo_tmp_spon_balance
             SET
      ...
      $sqlStr = 'DROP TABLE ngo_tmp_spon_balance;';
      $this->db->query($sqlStr);
   }
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
           "SELECT
            ...
            FROM ngo_sponsor_charges
               INNER JOIN ngo_admin_aco              ON spc_lACOID       = aco_lKeyID
               INNER JOIN ngo_sponsor                ON sp_lKeyID        = spc_lSponsorshipID
               INNER JOIN ngo_donors           ON sp_lForeignID    = pe_lKeyID
               LEFT  JOIN ngo_client_records         ON sp_lClientID     = cr_lKeyID
               LEFT  JOIN ngo_client_location        ON cr_lLocationID   = cl_lKeyID
               LEFT  JOIN ngo_sponsor_autocharge_log ON spc_lAutoGenID   = spcl_lKeyID
               INNER JOIN users AS usersC  ON spc_lOriginID    = usersC.id
               INNER JOIN users AS usersL  ON spc_lLastUpdateID= usersL.id
...
//               $this->chargeRec[$idx]->lACOID               = $row->spc_lACOID;
      ...
      $sqlStr =
           "SELECT
            ...
            FROM ngo_gifts
               ...
               INNER JOIN ngo_donors AS pDon    ON gi_lForeignID     = pDon.pe_lKeyID
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
        "SELECT
            ...
            spcl_lOriginID, first_name, last_name
         FROM ngo_sponsor_autocharge_log
            INNER JOIN users ON spcl_lOriginID=id
         WHERE 1 $strWhere
         LIMIT 0,1;";
      ...
      if ($numRows==0) {
         ...
         $clsACEntry->strUserSafe = htmlspecialchars($row->first_name.' '.$row->last_name);
      }
   }
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
           "INSERT INTO ngo_sponsor_autocharge_log
            SET
   ...
   //---------------------------------------------------------------------
      ...
      $mysqlAutoDate = strPrepDate(strtotime($lMonth.'/1/'.$lYear));
         ...
         //-----------------------------
      $sqlStr =
        "INSERT INTO ngo_sponsor_charges (
            spc_lAutoGenID,
            spc_lAutoGenACOID,
   ...
   //-----------------------------------------------------------------------
      ...
      $sqlStr =
        "SELECT
         ...
         FROM ngo_sponsor_charges
            INNER JOIN ngo_sponsor                    ON spc_lSponsorshipID   = sp_lKeyID
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
           "SELECT
            ...
            FROM ngo_gifts
               INNER JOIN ngo_admin_aco               ON gi_lACOID         = aco_lKeyID
               INNER JOIN ngo_sponsor                 ON sp_lKeyID         = gi_lSponsorID
               INNER JOIN ngo_donors AS pSpon   ON sp_lForeignID     = pSpon.pe_lKeyID
               INNER JOIN ngo_donors AS pDon    ON gi_lForeignID     = pDon.pe_lKeyID
   ...
   //---------------------------------------------------------------------
      ...
      if ($bReport){
         ...
         $sqlStr = 'DROP TABLE IF EXISTS ngo_tmpPastDue;';
         $this->db->query($sqlStr);
         $sqlStr =
              'CREATE TEMPORARY TABLE ngo_tmpPastDue (  -- TEMPORARY
         ...
         if ($lNumSponsors > 0){
            ...
            $sqlStr =
               'SELECT
                ...
                FROM ngo_tmpPastDue
                   INNER JOIN ngo_sponsor                    ON sp_lKeyID            = pd_lKeyID
                   ...
                   INNER JOIN ngo_lists_sponsorship_programs ON sc_lKeyID            = sp_lSponsorProgramID

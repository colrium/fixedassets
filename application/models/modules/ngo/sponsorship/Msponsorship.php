...
/*---------------------------------------------------------------------
// copyright (c) 2011-2015  Collins Riungu
// Nairobi Kenya 78759
//
...
class Msponsorship extends CI_Model{
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
         ...
         FROM ngo_sponsor
            INNER JOIN ngo_donors  AS tblPeopleSpon ON sp_lForeignID     = tblPeopleSpon.pe_lKeyID
            INNER JOIN users   AS uc ON uc.id=sp_lOriginID
            INNER JOIN users   AS ul ON ul.id=sp_lLastUpdateID
            INNER JOIN ngo_admin_aco           ON aco_lKeyID  = sp_lCommitmentACO
            LEFT  JOIN ngo_donors  AS tblPeopleHon  ON sp_lHonoreeID        = tblPeopleHon.pe_lKeyID
            LEFT  JOIN ngo_client_records                 ON cr_lKeyID            = sp_lClientID
            LEFT  JOIN ngo_client_location                ON cr_lLocationID       = cl_lKeyID
            LEFT  JOIN ngo_lists_sponsorship_programs     ON sp_lSponsorProgramID = sc_lKeyID
            LEFT  JOIN ngo_lists_generic AS listInactive  ON sp_lInactiveCatID    = listInactive.lgen_lKeyID
            LEFT  JOIN ngo_lists_generic AS listAttrib    ON sp_lAttributedTo     = listAttrib.lgen_lKeyID
...
//                      htmlspecialchars($sRec->strSponsorLNameHon.', '.$sRec->strSponsorFNameHon);
   ...
   //-----------------------------------------------------------------------
      ...
      $clsRecycle->strTable        = 'ngo_sponsor';
      ...
      $sqlStr =
        "SELECT
         ...
         FROM ngo_sponsor_charges
            ...
            INNER JOIN ngo_donors AS spon  ON sp_lForeignID=spon.pe_lKeyID
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
        'SELECT '.strExportFields_SponsorPayments()."
         FROM ngo_gifts
            INNER JOIN ngo_donors AS payer       ON gi_lForeignID          = payer.pe_lKeyID
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
        "SELECT sp_lKeyID
         FROM ngo_sponsor
            INNER JOIN ngo_client_records ON sp_lClientID=cr_lKeyID
         WHERE
            NOT sp_bRetired

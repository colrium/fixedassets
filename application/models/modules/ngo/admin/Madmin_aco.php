...
class Madmin_aco extends CI_Model{
   ...
   //---------------------------------------------------------------------
      $sqlStr =
           "SELECT pff_lCurrencyACO
            FROM ngo_uf_fields
            WHERE pff_lKeyID=?";
      $query = $this->db->query($sqlStr, array($lFieldID));
      ...
      $sqlStr =
         "UPDATE ngo_admin_aco
          SET
             ...
             aco_bDefault= ? 
          WHERE aco_lKeyID= ?;";
      $query = $this->db->query($sqlStr, array(($bDefault ? '1' : '0'), $lACOID));
   }
   ...
   //---------------------------------------------------------------------
      ...
      if ($bAll){
         ...
         $strOut .=
            '<td '.$strStyle.'>'
           .'<div class="radio radio-success"><label><input type="radio" name="'.$strRadioName.'" value="-1" '
               .$strChecked.'>All</label></div></td>'."\n";
      }
      ...
      foreach ($this->countries as $clsCountry){
         ...
         $strOut .=
            '<td '.$strStyle.'>'
           .'<div class="radio radio-success"><label><input type="radio" name="'.$strRadioName.'" value="'.$lKeyID.'" '
               .$strChecked.'>'.$clsCountry->strFlagImg."</label</td>\n";
      }
   ...
   //---------------------------------------------------------------------
      $sqlStr =
        'SELECT aco_lKeyID
         FROM ngo_admin_aco
         WHERE aco_bDefault = ?;';
      $query = $this->db->query($sqlStr, array(1));
      $numRows = $query->num_rows();
      if ($numRows==0) {
         return(null);
      }else {

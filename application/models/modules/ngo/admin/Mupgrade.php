...
class Mupgrade extends CI_Model{
   ...
   private function upgradeDBLevel($strNewVersionLevel, $strComment){
      $sqlStr =
         "INSERT INTO ngo_admin_version
             SET av_sngVersion  = ?,
             av_strVersionNotes = ?,
             av_dteInstalled    = NOW();";
      $this->db->query($sqlStr, array($strNewVersionLevel, strPrepStr($strComment)));
   }
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr = "
          ALTER TABLE  `ngo_lists_generic` CHANGE  `lgen_enumListType`  `lgen_enumListType`
      ...
      $this->db->query($sqlStr);
         ...
         /* ------------------------------------------------------
            ...
            ------------------------------------------------------ */
      $sqlStr = "
            ALTER TABLE  `ngo_vol_events_dates_shifts_assign`
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
        "CREATE TABLE IF NOT EXISTS `ngo_creport_dir` (
      ...
      $sqlStr =
        "CREATE TABLE IF NOT EXISTS `ngo_gifts_auctions` (
      ...
      $sqlStr =
        "CREATE TABLE IF NOT EXISTS `ngo_gifts_auctions_bidsheets` (
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
         "ALTER TABLE users
            CHANGE username  username VARCHAR( 120 )
      ...
      $sqlStr =
            'UPDATE users
             ...
             WHERE NOT us_bVolAccount AND NOT us_bAdmin AND NOT active;';
      $this->db->query($sqlStr);
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
          "ALTER TABLE ngo_groups_child DROP INDEX gc_lGroupID_2,
      ...
      $sqlStr =
        "ALTER TABLE  ngo_groups_parent CHANGE  gp_enumGroupType  gp_enumGroupType
             ENUM('people',  'household',  'business', 'businessContact',  'volunteer',
                  'staff',  'sponsorship',  'gift',  'client',  'user',
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
         'ALTER TABLE  `users` ADD  `us_bUserAllowGrants` TINYINT( 1 ) NOT NULL DEFAULT  \'0\'
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
        "CREATE TABLE IF NOT EXISTS inv_items (
           ...
           ivi_lRemInventoryByID int(11) DEFAULT NULL COMMENT 'User who removed item from inventory; foreign key to table users',

           ivi_dteReportedLost   date DEFAULT NULL COMMENT 'Date reported lost (null for not lost)',
           ivi_lFlaggedLostByID  int(11)    DEFAULT NULL COMMENT 'id of user reporting item lost; foreign key to table users',
      ...
      $sqlStr =
        "CREATE TABLE IF NOT EXISTS inv_cico (
           ...
           icc_lCheckedOutByID   int(11)    NOT NULL DEFAULT '0' COMMENT 'foreign key to table users',
           icc_lCheckedInByID    int(11)    DEFAULT NULL COMMENT 'foreign key to table users',

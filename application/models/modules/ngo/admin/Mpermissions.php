...
class Mpermissions extends CI_Model{
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
        "SELECT
            active, username, us_bAdmin, us_bDebugger,
            us_bVolAccount
         FROM users
         WHERE id=?;";
      $query = $this->db->query($sqlStr, array($lUserID));
      ...
      if ($numRows==0) {
         ...
         $row = $query->row();
         $acctAccess->bInactive   = (boolean)$row->active;
         $acctAccess->strUserName = $row->username;
   ...
   //---------------------------------------------------------------------
      ...
      $query = $this->db->query($sqlStr, array($lFID, strPrepStr($enumContextType)));
      ...
      $sqlStr =  
        'SELECT gp_lKeyID 
         ...
         WHERE 
            ...
            AND gp_strGroupName= ? 
         LIMIT 0, 1;';
      $query = $this->db->query($sqlStr, array(strPrepStr(CENUM_CONTEXT_USER), strPrepStr($strUserGroupName)));

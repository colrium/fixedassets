...
class Morganization extends CI_Model{
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
         "SELECT
          ...
          FROM ngo_admin_chapters
            INNER JOIN users AS usersC ON ch_lOrigID       = usersC.id
            INNER JOIN users AS usersL ON ch_lLastUpdateID = usersL.id
            INNER JOIN ngo_admin_aco             ON ch_lDefaultACO   = aco_lKeyID
            INNER JOIN ngo_lists_tz              ON ch_lTimeZone     = tz_lKeyID
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
          'INSERT INTO ngo_admin_chapters
   ...
   public function strDefaultDateFormatRadio($strDDLName, $bMatchUS){
      $strOut = "\n"
         .'<div class="radio radio-success"><label><input  type="radio" name="'.$strDDLName.'" value="US" '.($bMatchUS ? 'checked' : '').'>US (m / d / Y)</label>
		 </div>&nbsp;'."\n"
         .'<div class="radio radio-success"><label><input type="radio"  name="'.$strDDLName.'" value="EU" '.($bMatchUS ? '' : 'checked').'>Europe/India (d / m / Y)</label></div>&nbsp;'."\n";
      return($strOut);
   }
}

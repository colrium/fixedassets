...
class Mimport extends CI_Model{
   ...
   */
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
           "SELECT il_lKeyID, il_enumImportType, il_lOriginID,
            ...
            FROM ngo_import_log
               INNER JOIN users AS usersC ON il_lOriginID = usersC.id
   ...
   public function loadForeignIDs($lImportID){
      ...
      $sqlStr = "SELECT iid_lForeignID FROM ngo_import_ids WHERE iid_lImportID=$lImportID;";
      $query = $this->db->query($sqlStr);

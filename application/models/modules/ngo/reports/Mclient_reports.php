...
class Mclient_reports extends CI_Model{
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
        "SELECT cr_lKeyID
            FROM ngo_client_records $strInner
            WHERE 1
            AND NOT cr_bRetired
      ...
      $sqlStr =
        'SELECT '.strExportFields_Client()."
            FROM ngo_client_records
               ...
               LEFT  ngo_JOIN lists_generic               ON cr_lAttributedTo       = lgen_lKeyID
            WHERE 1
            AND NOT cr_bRetired
   ...
   //---------------------------------------------------------------------
      ...
      sqlQualClientViaStatus($sqlWhereSubquery, $strInner, false,
                       true,              false,     false);
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
        "SELECT cr_lKeyID
            FROM ngo_client_records 
               $strInner
               INNER JOIN ngo_client_status_cats ON csc_lKeyID=cst_lClientStatusCatID
            WHERE 1
            AND NOT cr_bRetired
            $sqlWhereSubquery
            $strStatCatIDWhere

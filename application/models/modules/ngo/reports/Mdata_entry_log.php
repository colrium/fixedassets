...
class Mdata_entry_log extends CI_Model{
   ...
   //---------------------------------------------------------------------   
      foreach ($this->ccprogs->cprogs as $cp){
         ...
         $sqlStr =
              "SELECT COUNT(*) AS lNumRecs, id, last_name, first_name
               FROM $strAETab
                  INNER JOIN users ON id=$strFNP"."_lOriginID
         ...
         case 'individual':
            ...
            $sqlStr =
              "SELECT COUNT(*) AS lNumRecs, id, last_name, first_name
               FROM ngo_client_records
                  INNER JOIN users ON id = cr_lOriginID
            ...
            if ($numRows > 0){
               ...
               foreach ($query->result() as $row){
                  ...
                  $en->lUserID      = $row->id;
                  $en->strLastName  = $row->last_name;
                  $en->strFirstName = $row->first_name;
   ...
   //---------------------------------------------------------------------
      $sqlStr =
        "SELECT COUNT(cr_lKeyID) AS lNumRecs
         FROM ngo_client_records
            INNER JOIN users ON id = cr_lOriginID
            INNER JOIN ngo_groups_child ON id =  gc_lForeignID

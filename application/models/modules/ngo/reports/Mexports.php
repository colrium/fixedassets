...
class Mexports extends CI_Model{
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
         'SELECT '.strExportFields_People().",
          ...
          FROM ngo_donors AS peepTab
             INNER JOIN ngo_donors AS houseTab ON peepTab.pe_lHouseholdID=houseTab.pe_lKeyID
             INNER JOIN ngo_admin_aco     ON peepTab.pe_lACO=aco_lKeyID
             LEFT  JOIN ngo_lists_generic ON peepTab.pe_lAttributedTo=lgen_lKeyID
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
         'SELECT '.strExportFields_Biz().",
          ...
          FROM ngo_donors AS bizTab
             INNER JOIN ngo_admin_aco     ON bizTab.pe_lACO=aco_lKeyID
             LEFT  JOIN ngo_lists_generic AS listAttrib ON bizTab.pe_lAttributedTo  = listAttrib.lgen_lKeyID
             LEFT  JOIN ngo_lists_generic AS listBizCat ON bizTab.pe_lBizIndustryID = listBizCat.lgen_lKeyID
          WHERE
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
         ...
         FROM ngo_biz_contacts
            INNER JOIN ngo_donors AS peepTab ON bc_lContactID = peepTab.pe_lKeyID
            INNER JOIN ngo_donors AS bizTab  ON bc_lBizID     = bizTab.pe_lKeyID
            LEFT  JOIN ngo_lists_generic AS listBizRelCat ON bc_lBizContactRelID=listBizRelCat.lgen_lKeyID
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
        "SELECT
            ...
            CONCAT(ackUser.first_name, ' ', ackUser.last_name) AS `Acknowledged By`

         FROM `ngo_gifts_hon_mem_links`
            INNER JOIN `ngo_lists_hon_mem`             ON ghm_lKeyID = ghml_lHonMemID
            INNER JOIN ngo_gifts                       ON gi_lKeyID  = ghml_lGiftID
            INNER JOIN ngo_admin_aco                   ON gi_lACOID  = aco_lKeyID
            INNER JOIN ngo_donors AS peepDonor   ON gi_lForeignID      = peepDonor.pe_lKeyID
            INNER JOIN ngo_donors AS peepHM      ON ghm_lFID           = peepHM.pe_lKeyID
            LEFT  JOIN ngo_donors AS peepMC      ON ghm_lMailContactID = peepMC.pe_lKeyID

            LEFT JOIN users AS ackUser       ON ackUser.id=ghml_lAckByID
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
         'SELECT '.strExportFields_Sponsor().",
          ...
          FROM ngo_sponsor
             INNER JOIN ngo_donors AS sponpeep   ON sponpeep.pe_lKeyID   = sp_lForeignID
             INNER JOIN ngo_lists_sponsorship_programs ON sp_lSponsorProgramID = sc_lKeyID
             INNER JOIN ngo_admin_aco AS commitACO     ON commitACO.aco_lKeyID = sp_lCommitmentACO
             LEFT  JOIN ngo_client_records             ON cr_lKeyID            = sp_lClientID
             LEFT  JOIN ngo_client_location            ON cr_lLocationID       = cl_lKeyID
             LEFT  JOIN ngo_lists_client_vocab         ON cr_lVocID            = cv_lKeyID
             LEFT  JOIN ngo_donors AS honpeep    ON honpeep.pe_lKeyID    = sp_lHonoreeID
             LEFT  JOIN ngo_lists_generic AS atab      ON sp_lAttributedTo     = lgen_lKeyID
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
         "SELECT
             ...
             sponcontact.pe_strCity    AS `Contact City`,       sponcontact.pe_strState AS `Contact State`,
             sponcontact.pe_strCountry AS `Contact Country`,    sponcontact.pe_strZip   AS `Contact Zip`,
             ...
             sponpeep.pe_strCity    AS `Sponsor City`,       sponpeep.pe_strState AS `Sponsor State`,
             sponpeep.pe_strCountry AS `Sponsor Country`,    sponpeep.pe_strZip   AS `Sponsor Zip`,
          ...
          FROM $strTmpTable
             INNER JOIN ngo_sponsor ON tmp_lSponID=sp_lKeyID
             INNER JOIN ngo_donors AS sponcontact   ON sponcontact.pe_lKeyID   = tmp_lContactID
             INNER JOIN ngo_donors AS sponpeep      ON sponpeep.pe_lKeyID      = sp_lForeignID
             INNER JOIN ngo_lists_sponsorship_programs    ON sp_lSponsorProgramID    = sc_lKeyID
             INNER JOIN ngo_admin_aco AS commitACO        ON commitACO.aco_lKeyID    = sp_lCommitmentACO
             INNER JOIN ngo_client_records                ON cr_lKeyID               = sp_lClientID
             INNER JOIN ngo_client_location               ON cr_lLocationID          = cl_lKeyID
             INNER JOIN ngo_lists_client_vocab            ON cr_lVocID               = cv_lKeyID
             LEFT  JOIN ngo_donors AS honpeep       ON honpeep.pe_lKeyID       = sp_lHonoreeID
             LEFT  JOIN ngo_client_status                 ON tmp_lStatusID           = csh_lKeyID
             LEFT  JOIN ngo_lists_client_status_entries   ON csh_lStatusID           = cst_lKeyID
             LEFT  JOIN ngo_client_status_cats            ON cst_lClientStatusCatID  = csc_lKeyID
          WHERE NOT sponpeep.pe_bRetired
             AND NOT sp_bRetired
          ORDER BY sp_lKeyID;";
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
         'SELECT '.strExportFields_Client()."
          FROM ngo_client_records
             INNER JOIN ngo_client_location             ON cr_lLocationID         = cl_lKeyID
             INNER JOIN ngo_client_status_cats          ON cr_lStatusCatID        = csc_lKeyID
             INNER JOIN ngo_lists_client_vocab          ON cr_lVocID              = cv_lKeyID
             INNER JOIN ngo_client_status               ON csh_lClientID          = cr_lKeyID
             INNER JOIN ngo_lists_client_status_entries ON csh_lStatusID          = cst_lKeyID
             LEFT  JOIN ngo_lists_generic               ON cr_lAttributedTo       = lgen_lKeyID

          --   INNER JOIN ngo_client_status_cats          ON cst_lClientStatusCatID = curStatSCat.csc_lKeyID
          --   INNER JOIN ngo_client_status_cats AS defSCat      ON cr_lStatusCatID        = defSCat.csc_lKeyID
   ...
   //---------------------------------------------------------------------
      ...
      switch ($enumTType){
         ...
         case CENUM_CONTEXT_CLIENT:
            if ($bInner){
               $strFrom = "FROM ngo_client_records
                              LEFT JOIN $strJoinTable ON cr_lKeyID = $strForeignIDFN \n";
            }else {
               $strFrom = "FROM $strJoinTable
                              INNER JOIN ngo_client_records ON cr_lKeyID = $strForeignIDFN \n";
            }
            $strOut = 'SELECT '.$this->strSelectExtraViaContext($enumTType).' '
                          .$strFieldNamesSel."
                       $strFrom
         ...
         case CENUM_CONTEXT_LOCATION:
            if ($bInner){
               $strFrom = "FROM ngo_client_location
                              LEFT JOIN $strJoinTable ON cl_lKeyID = $strForeignIDFN \n";
            }else {
               $strFrom = "FROM $strJoinTable
                              INNER JOIN ngo_client_location ON cl_lKeyID = $strForeignIDFN \n";
            }
            $strOut = 'SELECT '.$this->strSelectExtraViaContext($enumTType).' '
                          .$strFieldNamesSel."
                       $strFrom
         ...
         case CENUM_CONTEXT_GIFT:
            if ($bInner){
               ...
               $strFrom = "FROM $strJoinTable
                              INNER JOIN ngo_donors  ON gi_lForeignID = pe_lKeyID
                              INNER JOIN ngo_admin_aco     ON gi_lACOID     = aco_lKeyID
                              INNER JOIN ngo_gifts         ON gi_lKeyID = $strForeignIDFN \n";
            }
            $strOut = 'SELECT '.$this->strSelectExtraViaContext($enumTType).' '
                          .$strFieldNamesSel."
                       $strFrom
         ...
         case CENUM_CONTEXT_SPONSORSHIP:
            if ($bInner){
               $strFrom = "FROM ngo_sponsor
                              INNER JOIN ngo_donors      ON pe_lKeyID = sp_lForeignID
                              LEFT  JOIN ngo_client_records    ON cr_lKeyID = sp_lClientID
                              LEFT  JOIN $strJoinTable     ON sp_lKeyID = $strForeignIDFN \n";
            }else {
               $strFrom = "FROM $strJoinTable
                              INNER JOIN ngo_sponsor           ON sp_lKeyID = $strForeignIDFN
                              INNER JOIN ngo_donors      ON pe_lKeyID = sp_lForeignID
                              LEFT  JOIN ngo_client_records    ON cr_lKeyID = sp_lClientID \n";
            }
            $strOut = 'SELECT '.$this->strSelectExtraViaContext($enumTType).' '

<?php
/*---------------------------------------------------------------------
  copyright (c) 2014 by Mutugi Riungu
...
class Mimg_doc_tags extends CI_Model{
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
        "SELECT
         ...
         FROM ngo_doc_img_tag_ddl_multi
            INNER JOIN ngo_doc_img_tag_ddl ON dim_lDDLID=dit_lKeyID
   ...
   //---------------------------------------------------------------------
      $sqlStr =
        'INSERT INTO ngo_doc_img_tag_ddl
         SET
   ...
   //---------------------------------------------------------------------
      ...
      $this->removeImageTags($lImgDocID);
         ...
         // in with the new
      if (count($tagIDs) > 0){
         $sqlBase = "INSERT INTO doc_img_tag_ddl_multi SET dim_lImgDocID=".$this->db->escape($lImgDocID).", dim_lDDLID=";
   ...
   //---------------------------------------------------------------------
      $sqlStr =
        "SELECT COUNT(*) AS lNumImgDocs
         FROM ngo_doc_img_tag_ddl_multi
            INNER JOIN ngo_docs_images ON dim_lImgDocID=di_lKeyID

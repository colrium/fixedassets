...
class Mimage_doc extends CI_Model{
   ...
   //---------------------------------------------------------------------
      ...
      $this->sqlJoinExtra =
            " INNER JOIN ngo_doc_img_tag_ddl_multi ON dim_lImgDocID=di_lKeyID \n"
   ...
   function loadDocImageInfoViaContextFID($enumContextType, $lFID){
      ...
      $this->loadDocImageGeneric($bIncludeContext=false, $enumContext=null);
   }
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
         ...
         FROM ngo_docs_images
            $this->sqlJoinExtra
            INNER JOIN users  AS uc ON uc.id=di_lOriginID
            INNER JOIN users  AS ul ON ul.id=di_lLastUpdateID
         WHERE NOT di_bRetired $this->sqlWhereExtra
         $this->sqlSort
         $this->sqlLimit;";

      $query = $this->db->query($sqlStr);
      $this->lNumImageDocs =  $query->num_rows();
      $lNumRows = $this->lNumImageDocs;
      if ($lNumRows == 0){
   ...
   //---------------------------------------------------------------------
      switch ($enumContext){
         ...
         case CENUM_CONTEXT_STAFF:
            $strOut =
              ' staff.last_name AS strLastName, staff.first_name AS strFirstName ';
            break;
   ...
   //---------------------------------------------------------------------
      switch ($enumContext){
         case CENUM_CONTEXT_CLIENT:
            $strOut =
              'INNER JOIN ngo_client_records ON cr_lKeyID=di_lForeignID '."\n";
            break;

         case CENUM_CONTEXT_PEOPLE:
            $strOut =
              'INNER JOIN ngo_donors ON pe_lKeyID=di_lForeignID '."\n";
            break;

         case CENUM_CONTEXT_LOCATION:
            $strOut =
              'INNER JOIN ngo_client_location ON cl_lKeyID=di_lForeignID '."\n";
            break;

         case CENUM_CONTEXT_GRANTPROVIDER:
            $strOut =
              'INNER JOIN ngo_grant_providers ON gpr_lKeyID=di_lForeignID '."\n";
            break;

         case CENUM_CONTEXT_INVITEM:
            $strOut =
              'INNER JOIN ngo_inv_items ON ivi_lKeyID=di_lForeignID '."\n";
            break;

         case CENUM_CONTEXT_AUCTION:
            $strOut =
              'INNER JOIN ngo_gifts_auctions ON auc_lKeyID=di_lForeignID '."\n";
            break;

         case CENUM_CONTEXT_AUCTIONITEM:
            $strOut =
              'INNER JOIN ngo_gifts_auctions_items    ON ait_lKeyID = di_lForeignID '."\n"
             .'INNER JOIN ngo_gifts_auctions_packages ON ap_lKeyID  = ait_lPackageID '."\n"
             .'INNER JOIN ngo_gifts_auctions          ON auc_lKeyID = ap_lAuctionID '."\n";
            break;

         case CENUM_CONTEXT_AUCTIONPACKAGE:
            $strOut =
              'INNER JOIN ngo_gifts_auctions_packages ON ap_lKeyID  = di_lForeignID '."\n"
             .'INNER JOIN ngo_gifts_auctions          ON auc_lKeyID = ap_lAuctionID '."\n";
            break;
         ...
         case CENUM_CONTEXT_SPONSORSHIP:
            $strOut =
              'INNER JOIN ngo_sponsor      ON sp_lKeyID=di_lForeignID '."\n"
             .'INNER JOIN ngo_donors ON pe_lKeyID=sp_lForeignID '."\n";
            break;

         case CENUM_CONTEXT_ORGANIZATION:
            $strOut =
              'INNER JOIN ngo_admin_chapters ON ch_lKeyID=di_lForeignID '."\n";
            break;

         case CENUM_CONTEXT_STAFF:
            $strOut =
              'INNER JOIN users AS staff ON staff.id=di_lForeignID '."\n";
            break;

         case CENUM_CONTEXT_VOLUNTEER:
            $strOut =
              'INNER JOIN ngo_volunteers   ON vol_lKeyID = di_lForeignID

...
class Mtime_sheets extends CI_Model{
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
        "SELECT
         ...
         FROM ngo_staff_timesheets
            INNER JOIN users   AS uc ON uc.id=ts_lOriginID
            INNER JOIN users   AS ul ON ul.id=ts_lLastUpdateID
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
          'INSERT INTO ngo_staff_timesheets
...
*/
   ...
   //---------------------------------------------------------------------
      $sqlStr =
           "DELETE FROM ngo_staff_ts_staff
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
         'INSERT INTO ngo_staff_ts_staff (
...
*/
   ...
   //---------------------------------------------------------------------
      $sqlStr =
           "DELETE FROM ngo_staff_ts_admin WHERE 1;";
      $query = $this->db->query($sqlStr);
   }
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
         'INSERT INTO ngo_staff_ts_admin (
   ...
   //---------------------------------------------------------------------
      ...
      if ($this->sqlOrder == ''){
         $strOrder = ' uts.last_name, uts.first_name, tsl_lStaffID, tsl_dteSubmitted DESC ';
      }else {
      ...
      $sqlStr =
        "SELECT
            ...
            uts.first_name AS strUserFName, uts.last_name AS strUserLName,
         ...
         FROM ngo_staff_ts_log
            INNER JOIN ngo_staff_timesheets ON ts_lKeyID=tsl_lTimeSheetID
            INNER JOIN users   AS uc  ON uc.id=ts_lOriginID
            INNER JOIN users   AS ul  ON ul.id=ts_lLastUpdateID
            INNER JOIN users   AS uts ON uts.id=tsl_lStaffID
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
        "SELECT
            ...
            uc.first_name AS strUCFName, uc.last_name AS strUCLName,
            ul.first_name AS strULFName, ul.last_name AS strULLName
         FROM ngo_staff_ts_log_entry
            INNER JOIN ngo_staff_ts_log  ON tsle_lTSLogID=tsl_lKeyID
            INNER JOIN ngo_groups_parent ON gp_lKeyID = tsle_lLocationID
            INNER JOIN users   AS uc  ON uc.id=tsle_lOriginID
            INNER JOIN users   AS ul  ON ul.id=tsle_lLastUpdateID
         WHERE 1 $this->sqlWhere
...
$le->strDteLogEntry = date('m/d/Y H:i:s', $le->dteLogEntry);
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
        'INSERT INTO ngo_staff_ts_log_entry
         SET
   ...
   //---------------------------------------------------------------------
      $sqlStr =
        "DELETE FROM ngo_staff_ts_log_entry
         WHERE tsle_lKeyID = $lEntryID;";
      $query = $this->db->query($sqlStr);
   }
...
*/
   ...
   //---------------------------------------------------------------------
      $sqlStr =
        'SELECT
         ...
         FROM ngo_staff_ts_staff
            INNER JOIN ngo_staff_timesheets ON tss_lTimeSheetID=ts_lKeyID
            INNER JOIN users ON tss_lStaffID=id
         WHERE NOT ts_bRetired
         ORDER BY last_name, first_name, tss_lKeyID;';
      ...
      if ($lNumUsers > 0) {
         ...
         foreach ($query->result() as $row) {
            ...
            $user->strLastName          = $row->last_name;
            $user->strFirstName         = $row->first_name;
   ...
   //---------------------------------------------------------------------
      $sqlStr =
        "SELECT SUM(tspr_lMinutesToProject) AS lNumMinutes
         FROM ngo_staff_ts_log_projects
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
        "SELECT tspr_lKeyID, tspr_lTSLogID, tspr_lProjectID,
         ...
         FROM ngo_staff_ts_log_projects
            INNER JOIN ngo_groups_parent ON tspr_lProjectID=gp_lKeyID
         WHERE tspr_lTSLogID = $lTSLogID
   ...
   //---------------------------------------------------------------------
         // out with the old
      $sqlStr =
         "DELETE FROM ngo_staff_ts_log_projects
          WHERE tspr_lTSLogID=$lTSLogID;";
      $query = $this->db->query($sqlStr);
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
        'INSERT INTO ngo_staff_ts_log_projects
         SET

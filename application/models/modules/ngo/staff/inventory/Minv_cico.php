   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
        "SELECT
            ...
            icc_lCheckedOutByID,
            icc_lCheckedInByID,
            uco.first_name   AS strCheckOutFName,  uco.last_name   AS strCheckOutLName,
            uci.first_name   AS strCheckInFName,   uci.last_name   AS strCheckInLName

            FROM ngo_inv_cico
               INNER JOIN users  AS uc    ON uc.id    = icc_lOriginID
               INNER JOIN users  AS ul    ON ul.id    = icc_lLastUpdateID
               INNER JOIN users  AS uco   ON uco.id   = icc_lCheckedOutByID
               LEFT  JOIN users  AS uci   ON uci.id   = icc_lCheckedInByID

               INNER JOIN ngo_inv_items             ON icc_lItemID     = ivi_lKeyID
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
        "SELECT
         ...
         FROM ngo_inv_cico
            INNER JOIN ngo_inv_items    ON icc_lItemID=ivi_lKeyID
            INNER JOIN ngo_inv_cats     ON ivi_lCategoryID=ivc_lKeyID
            INNER JOIN ngo_$strTmpTable ON tmp_lICatID=ivc_lKeyID

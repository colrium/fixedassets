...
/*---------------------------------------------------------------------
// ActionAid CRM
// copyright (c) 2015  Collins Riungu
//
   ...
   //---------------------------------------------------------------------
      $sqlStr =
        "SELECT
            ...
            ivc_lOriginID, ivc_lLastUpdateID, ivc_dteOrigin, ivc_dteLastUpdate
         FROM ngo_inv_cats
            LEFT JOIN ngo_inv_items ON ivi_lCategoryID=ivc_lKeyID
         WHERE NOT ivc_bRetired
...
*/
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
        "SELECT
            ...
            uc.first_name    AS strCFName,      uc.last_name    AS strCLName,
            ul.first_name    AS strLFName,      ul.last_name    AS strLLName,
            ur.first_name    AS strRemInvFName, ur.last_name    AS strRemInvLName,
            ulost.first_name AS strLostFName,   ulost.last_name AS strLostLName

         FROM ngo_inv_items
               INNER JOIN users  AS uc    ON uc.id    = ivi_lOriginID
               INNER JOIN users  AS ul    ON ul.id    = ivi_lLastUpdateID
               LEFT  JOIN users  AS ur    ON ur.id    = ivi_lRemInventoryByID
               LEFT  JOIN users  AS ulost ON ulost.id = ivi_lFlaggedLostByID
               INNER JOIN ngo_inv_cats              ON ivc_lKeyID      = ivi_lCategoryID
               INNER JOIN ngo_admin_aco             ON ivi_lACOID      = aco_lKeyID
   ...
   //---------------------------------------------------------------------
         // remove history
      $sqlStr = "DELETE FROM ngo_inv_history WHERE ih_lItemID=$lIItemID;";
      $this->db->query($sqlStr);

         // remove cico
      $sqlStr = "DELETE FROM ngo_inv_cico WHERE icc_lItemID=$lIItemID;";
      $this->db->query($sqlStr);

         // remove item
      $sqlStr = "DELETE FROM ngo_inv_items WHERE ivi_lKeyID=$lIItemID;";
      $this->db->query($sqlStr);
   }
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
        "SELECT
            ...
            ih_dteLastUpdate,
            uc.first_name AS strCFName,
            uc.last_name  AS strCLName
         FROM ngo_inv_history
               INNER JOIN users  AS uc    ON uc.id = ih_lOriginID

...
/*---------------------------------------------------------------------
// copyright (c) 2014  Collins Riungu
//
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
        "SELECT
            ...
            rpt.first_name AS strRptFName, rpt.last_name AS strRptLName,
            uc.first_name  AS strUCFName,  uc.last_name  AS strUCLName,
            ul.first_name  AS strULFName,  ul.last_name  AS strULLName

         FROM ngo_aayhf_staff_status
            INNER JOIN users   AS rpt ON rpt.id = ss_lUserID
            INNER JOIN users   AS uc  ON uc.id  = ss_lOriginID
            INNER JOIN users   AS ul  ON ul.id  = ss_lLastUpdateID
...
// ------------------------------------- */
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
         'INSERT INTO ngo_aayhf_staff_status
   ...
   //---------------------------------------------------------------------
      $sqlStr =
        "DELETE FROM ngo_aayhf_staff_status
         WHERE ss_lKeyID=$lSReportID;";
      $query = $this->db->query($sqlStr);

         // delete entries
      $sqlStr =
        "DELETE FROM ngo_aayhf_staff_status_details
   ...
   //---------------------------------------------------------------------
      $sqlStr =
        "SELECT
         ...
         FROM ngo_aayhf_staff_status_details
            INNER JOIN users   AS uc  ON uc.id  = ssd_lOriginID
            INNER JOIN users   AS ul  ON ul.id  = ssd_lLastUpdateID
   ...
   //---------------------------------------------------------------------
      $glUserID = USERID;
      $sqlStr =
         'INSERT INTO ngo_aayhf_staff_status_details
   ...
   //---------------------------------------------------------------------
      $sqlStr =
         'UPDATE ngo_aayhf_staff_status_details
          SET '.$this->statEntrySQLCommon($entry)."
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
         "INSERT INTO $strTmpTable (tmp_lUserID, tmp_lGroupID)
             SELECT
                id, gp_lKeyID
             FROM users
                INNER JOIN groups_child ON id=gc_lForeignID
      ...
      $sqlStr =
         "INSERT INTO $strTmpTable (tmp_lUserID)
            SELECT id
            FROM users AS A
            LEFT JOIN $strTmpTable AS B ON A.id = B.tmp_lUserID
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
         "SELECT id, first_name, last_name,
          ...
          FROM $strTmpTable
             INNER JOIN users   ON id    = tmp_lUserID
             LEFT  JOIN groups_parent ON tmp_lGroupID = gp_lKeyID
          WHERE 1
          ORDER BY gp_strGroupName, gp_lKeyID, last_name, first_name, id;";
      ...
      if ($lNumRows > 0) {
         ...
         foreach ($query->result() as $row){
            ...
            $usr->lUserID  = $row->id;
            $usr->strFName = $row->first_name;
            $usr->strLName = $row->last_name;
            ++$sg->lNumUsers;
         }
      }
/* -------------------------------------
...
// ------------------------------------- */
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
        "SELECT
         ...
         FROM ngo_aayhf_staff_status_review
            INNER JOIN users   AS uc  ON uc.id  = ssr_lOriginID
            INNER JOIN users   AS ul  ON ul.id  = ssr_lLastUpdateID
         WHERE 1 $this->sqlWhereReviews
         ORDER BY  uc.last_name, uc.first_name, ssr_lKeyID;";
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
         'INSERT INTO ngo_aayhf_staff_status_review
   ...
   //---------------------------------------------------------------------
      $sqlStr =
         'UPDATE ngo_aayhf_staff_status_review
          SET '.$this->sqlCommonRptReview($mgrReview)."
   ...
   */
   ...
   //---------------------------------------------------------------------
      $sqlStr =
        "SELECT COUNT(*) AS lNumRecs
         FROM ngo_aayhf_staff_status
            INNER JOIN ngo_aayhf_staff_status_review ON ssr_lStatusID=ss_lKeyID

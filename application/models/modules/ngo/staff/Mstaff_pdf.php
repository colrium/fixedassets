...
class Mstaff_pdf extends mstaff_status{
   ...
   //---------------------------------------------------------------------
      ...
      $users->sqlWhere = " AND id IN ($strUserIDs) ";
      ...
      foreach ($users->userRec as $urec){
         ...
         if ($this->lNumSReports == 0){
            ...
            $pdf->Ln(10);
            $pdf->Cell($gpdf_MarginLR,10,'There are no status reports for '.$urec->first_name.' '.$urec->last_name);
         }else {
   ...
   //---------------------------------------------------------------------
      ...
      foreach ($sRpt->lStaffGroup as $lGroupID){
         ...
         if ($sRpt->strSort == 'date'){
            $this->sqlOrder = ' ss_dteSubmitDate, rpt.last_name, rpt.first_name ';
         }else {
            $this->sqlOrder = ' rpt.last_name, rpt.first_name, ss_dteSubmitDate ';
         }
         $this->loadStatusReports();

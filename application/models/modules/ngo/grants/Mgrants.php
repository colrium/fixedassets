...
class Mgrants extends CI_Model{
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
           "SELECT
            ...
            FROM ngo_grant_providers
               INNER JOIN users  AS uc ON uc.id=gpr_lOriginID
               INNER JOIN users  AS ul ON ul.id=gpr_lLastUpdateID
               -- INNER JOIN admin_aco          ON gpr_lACO         = aco_lKeyID

               LEFT  JOIN ngo_lists_generic      ON gpr_lAttributedTo=lgen_lKeyID
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
         'INSERT INTO ngo_grant_providers
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
           "SELECT
            ...
            FROM ngo_grants
               INNER JOIN ngo_grant_providers    ON gr_lProviderID=gpr_lKeyID
               INNER JOIN users  AS uc ON uc.id  =gr_lOriginID
               INNER JOIN users  AS ul ON ul.id  =gr_lLastUpdateID
               INNER JOIN ngo_admin_aco          ON gr_lACO       = aco_lKeyID

               LEFT  JOIN ngo_lists_generic      ON gr_lAttributedTo=lgen_lKeyID

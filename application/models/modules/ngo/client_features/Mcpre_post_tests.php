...
/*---------------------------------------------------------------------
// copyright (c) 2014-2015  Collins Riungu
//
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
        "SELECT
         ...
         FROM ngo_cpp_tests
            INNER JOIN users   AS uc ON uc.id = cpp_lOriginID
            INNER JOIN users   AS ul ON ul.id = cpp_lLastUpdateID
            LEFT  JOIN ngo_lists_generic       ON cpp_lTestCat = lgen_lKeyID
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
        "SELECT
         ...
         FROM ngo_cpp_questions
            INNER JOIN ngo_cpp_tests           ON cpp_lKeyID   = cpq_lPrePostID
            INNER JOIN users   AS uc ON uc.id = cpq_lOriginID
            INNER JOIN users   AS ul ON ul.id = cpq_lLastUpdateID
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
        "SELECT
         ...
         FROM ngo_cpp_test_log
            INNER JOIN users AS uc ON uc.id=cptl_lOriginID
            INNER JOIN users AS ul ON ul.id=cptl_lLastUpdateID
...
*/
   ...
   //---------------------------------------------------------------------
      $sqlStr =
         "DELETE FROM ngo_cpp_test_results
   ...
   //---------------------------------------------------------------------
      ...
      $strInsert =
           'INSERT INTO ngo_cpp_test_results (cptr_lQuestionID, cptr_lTestLogID, cptr_bPreTest, cptr_bAnswerCorrect)
               VALUES ';
      foreach ($QandA as $QA){
         $strInsert .=
...
*/
   ...
   //---------------------------------------------------------------------
      $strOut = '<select class="form-control2 chosen" name="'.$strDDLName.'">'."\n";
...
//                        $QA->score->lPostTestRight + $QA->score->lPostTestWrong;
      ...
      $sqlStr =
          "SELECT COUNT(*) AS lNumRecs
           FROM $strTmpTableName
              INNER JOIN ngo_client_records            ON tmp_lClientID  = cr_lKeyID
   ...
   //-----------------------------------------------------------------------
      ...
      if (is_null($demoTable)){
         $strInner =
               'INNER JOIN ngo_client_records ON cr_lKeyID = cptl_lClientID '."\n";
      }else {
         ...
         $strInner =
            ...
            // grouped by race
         $strInnerGroup = $strInner.'LEFT JOIN ngo_uf_ddl ON ufddl_lKeyID='.$demoTable->fiRace->strFieldNameInternal."\n";
         ...
         $strInnerGroup = $strInner.'LEFT JOIN ngo_uf_ddl ON ufddl_lKeyID='.$demoTable->fiEthnicity->strFieldNameInternal."\n";
         $this->testsTakenGrouped($lPPTestID, $sRpt->strBetween, '', $strInnerGroup, $lNumGroups, $groups);
         $strOut .= $this->ppRptTotViaGroup('Tests Taken, by Ethnicity', $lNumGroups, $groups);

            // grouped by age
         $strInnerGroup = $strInner.'LEFT JOIN ngo_uf_ddl ON ufddl_lKeyID='.$demoTable->fiAgeRange->strFieldNameInternal."\n";
         $this->testsTakenGrouped($lPPTestID, $sRpt->strBetween, '', $strInnerGroup, $lNumGroups, $groups);
         $strOut .= $this->ppRptTotViaGroup('Tests Taken, by Age Range', $lNumGroups, $groups);

            // grouped by school (elementary/middle/high)
         $strInnerGroup = $strInner.'LEFT JOIN ngo_uf_ddl ON ufddl_lKeyID='.$demoTable->fiGradeLevel->strFieldNameInternal."\n";
         $this->testsTakenGrouped($lPPTestID, $sRpt->strBetween, '', $strInnerGroup, $lNumGroups, $groups);
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
        "SELECT COUNT(*) AS lNumRecs, cr_enumGender
         FROM ngo_cpp_test_log
            INNER JOIN ngo_client_records ON cptl_lClientID=cr_lKeyID
         WHERE
               NOT cptl_bRetired
            AND cptl_lPrePostID=$lTestID
            AND ((cptl_dtePreTest $strBetween) OR (cptl_dtePostTest $strBetween))

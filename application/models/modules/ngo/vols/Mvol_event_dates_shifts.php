...
class Mvol_event_dates_shifts extends CI_Model{
   ...
   //-----------------------------------------------------------------------
      $sqlStr =
        "SELECT COUNT(*) AS lNumRecs
         FROM ngo_vol_events_dates_shifts
   ...
   //-----------------------------------------------------------------------
      ...
      $sqlStr =
        "SELECT
         ...
         FROM ngo_vol_events_dates_shifts
            INNER JOIN ngo_vol_events_dates ON vs_lEventDateID = ved_lKeyID
            INNER JOIN ngo_vol_events       ON vem_lKeyID      = ved_lVolEventID
            LEFT  JOIN ngo_lists_generic    ON vs_lJobCode     = lgen_lKeyID
         WHERE
            NOT vs_bRetired
            $strWhere
   ...
   //-----------------------------------------------------------------------
      $clsES = &$this->shifts[0];
      $sqlStr =
         'INSERT INTO ngo_vol_events_dates_shifts
   ...
   //-----------------------------------------------------------------------
      $clsES = &$this->shifts[0];
      $sqlStr =
         'UPDATE ngo_vol_events_dates_shifts
          SET '.$this->sqlESCommon($clsES)."
   ...
   //-----------------------------------------------------------------------
      ...
      $sqlStr =
        "DELETE
         ...
         FROM ngo_vol_events_dates_shifts
            LEFT JOIN ngo_vol_events_dates_shifts_assign ON vs_lKeyID  = vsa_lEventDateShiftID
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
         ...
         FROM ngo_vol_events
            INNER JOIN ngo_vol_events_dates               ON vem_lKeyID = ved_lVolEventID
            INNER JOIN ngo_vol_events_dates_shifts        ON ved_lKeyID = vs_lEventDateID
            INNER JOIN ngo_vol_events_dates_shifts_assign ON vs_lKeyID  = vsa_lEventDateShiftID
         WHERE
            NOT vem_bRetired
            AND NOT vs_bRetired

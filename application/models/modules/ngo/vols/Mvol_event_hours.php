...
class Mvol_event_hours extends CI_Model{
      ...
      $sqlStr =
           "SELECT SUM(vsa_dHoursWorked) AS dHours
            FROM ngo_vol_events_dates_shifts_assign
               INNER JOIN ngo_vol_events_dates_shifts ON vsa_lEventDateShiftID = vs_lKeyID
               INNER JOIN ngo_vol_events_dates        ON vs_lEventDateID = ved_lKeyID
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
           "SELECT
            ...
            FROM ngo_vol_events_dates_shifts_assign
               INNER JOIN ngo_lists_generic ON vsa_lActivityID=lgen_lKeyID
            WHERE NOT vsa_bRetired
               AND vsa_lEventDateShiftID IS NULL
               AND vsa_lVolID=$lVolID
      ...
      if ($lNumUnscheduled > 0){
         ...
         foreach ($query->result() as $row){
            ...
            $act->dteActivityDate   = dteMySQLDate2Unix($row->vsa_dteActivityDate);
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
        "SELECT
         ...
         FROM ngo_vol_events_dates_shifts_assign
            INNER JOIN ngo_volunteers              ON vsa_lVolID            = vol_lKeyID
      ...
      if ($query->num_rows() == 0){
         ...
         foreach ($query->result() as $row){
            ...
            $this->volEHrs[$idx]->dteEvent            = dteMySQLDate2Unix($row->ved_dteEvent);
   ...
   function setVolHours($lVolShiftAssignID, $dHours){
      $sqlStr =
        'UPDATE ngo_vol_events_dates_shifts_assign
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
        "SELECT
         ...
         FROM ngo_vol_events_dates_shifts_assign
            INNER JOIN ngo_lists_generic AS act ON vsa_lActivityID    = act.lgen_lKeyID
            INNER JOIN users   AS uc ON uc.id = vsa_lOriginID
            INNER JOIN users   AS ul ON ul.id = vsa_lLastUpdateID
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr = '
         INSERT INTO ngo_vol_events_dates_shifts_assign
   ...
   //
   //---------------------------------------------------------------------
      $sqlStr = "DELETE FROM ngo_vol_events_dates_shifts_assign WHERE vsa_lKeyID=$lActivityID;";
      $this->db->query($sqlStr);
   }
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
           "SELECT
            ...
            FROM $strTabName
               INNER JOIN ngo_vol_events_dates_shifts_assign ON vsa_lKeyID            = tmp_lShiftID
               INNER JOIN ngo_vol_events_dates_shifts        ON vsa_lEventDateShiftID = vs_lKeyID
               INNER JOIN ngo_vol_events_dates               ON vs_lEventDateID       = ved_lKeyID
               INNER JOIN ngo_vol_events                     ON ved_lVolEventID       = vem_lKeyID
               INNER JOIN ngo_volunteers                     ON vsa_lVolID            = vol_lKeyID
               INNER JOIN ngo_donors AS peepTab        ON vol_lPeopleID         = peepTab.pe_lKeyID
            $strWhereOrder;";
      $query = $this->db->query($sqlStr);
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
          'SELECT
               ...
               .strExportFields_People()."
           FROM $strTabName
              INNER JOIN ngo_volunteers              ON tmp_lVolID    = vol_lKeyID
              INNER JOIN ngo_donors AS peepTab ON vol_lPeopleID = peepTab.pe_lKeyID
      ...
      $sqlStr =
           "SELECT
            ...
            FROM ngo_vol_events_dates_shifts_assign
               INNER JOIN ngo_lists_generic AS act     ON vsa_lActivityID = act.lgen_lKeyID
               INNER JOIN ngo_volunteers               ON vol_lKeyID      = vsa_lVolID
               INNER JOIN ngo_donors  AS peepTab ON pe_lKeyID       = vol_lPeopleID
               LEFT  JOIN ngo_lists_generic AS jc      ON vsa_lJobCode    = jc.lgen_lKeyID
            WHERE
               NOT vsa_bRetired
               AND vsa_lVolID=$lVolID
      ...
      $sqlStr =
           "SELECT
               ...
               vsa_strNotes AS `Notes`, "
                  .strExportFields_Vol()."
             FROM $strTab
                INNER JOIN ngo_vol_events_dates_shifts_assign ON vsa_lKeyID            = tmphr_lEventAssignID
                INNER JOIN ngo_volunteers                     ON tmphr_lVolID          = vol_lKeyID
                INNER JOIN ngo_donors AS peepTab        ON vol_lPeopleID         = peepTab.pe_lKeyID
                LEFT  JOIN ngo_vol_events_dates_shifts        ON vsa_lEventDateShiftID = vs_lKeyID
                LEFT  JOIN ngo_vol_events_dates               ON vs_lEventDateID       = ved_lKeyID
                LEFT  JOIN ngo_vol_events                     ON ved_lVolEventID       = vem_lKeyID
                LEFT  JOIN ngo_lists_generic                  ON vsa_lActivityID       = lgen_lKeyID

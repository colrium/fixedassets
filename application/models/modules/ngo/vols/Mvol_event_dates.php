...
class Mvol_event_dates extends CI_Model{
   ...
   //-----------------------------------------------------------------------
      ...
      $sqlStr =
          "INSERT INTO ngo_vol_events_dates
           SET
      ...
      $sqlStr =
        "SELECT
         ...
         FROM ngo_vol_events_dates
            LEFT JOIN ngo_vol_events_dates_shifts ON vs_lEventDateID=ved_lKeyID
         WHERE 1 $strWhere
         GROUP BY ved_lKeyID
   ...
   //-----------------------------------------------------------------------
      ...
      $sqlStr =
        "DELETE
         ...
         FROM ngo_vol_events_dates
            LEFT JOIN ngo_vol_events_dates_shifts        ON ved_lKeyID = vs_lEventDateID

   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
        'SELECT
         ...
         FROM ngo_uf_fields
            INNER JOIN ngo_uf_tables ON pff_lTableID= pft_lKeyID
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
        "SELECT
             ...
             ufddl_strDDLEntry
         FROM ngo_uf_ddl_multi
            INNER JOIN ngo_uf_ddl ON pdm_lDDLID=ufddl_lKeyID
         WHERE
   ...
   //---------------------------------------------------------------------
      ...
      if ($lNumEntries > 0) {
         ...
         foreach ($query->result() as $row){
            ...
            $sqlUpdate = "UPDATE ngo_uf_ddl SET ufddl_lSortIDX=$idx WHERE ufddl_lKeyID=$lKeyID;";

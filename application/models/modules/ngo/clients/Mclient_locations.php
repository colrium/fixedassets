   ...
   //---------------------------------------------------------------------
      $sqlStr =
        "SELECT
         ...
         FROM ngo_client_location
               INNER JOIN users  AS uc ON uc.id=cl_lOriginID
               INNER JOIN users  AS ul ON ul.id=cl_lLastUpdateID
...
//      }
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
         'INSERT INTO ngo_client_location
   ...
   //---------------------------------------------------------------------
      ...
      if ($bNoLocs){
         ...
         if ($gbAdmin){
            ...
            $strOutMsg .= 'Please contact your '.APPNAME.' administrator to add one or more client locations.<br><br>';
         }
      }
      return($bNoLocs);
   }
   ...
   //---------------------------------------------------------------------
      $sqlStr =
         "SELECT clsp_lKeyID
          FROM ngo_client_loc_supported_sponprogs
          WHERE
             clsp_lLocID=$lLocID
...
//      }
   ...
   //---------------------------------------------------------------------
      $sqlStr =
         "INSERT INTO ngo_client_loc_supported_sponprogs
          SET
             clsp_lLocID=$lLocID,
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
        "SELECT
            sc_lKeyID
         FROM ngo_lists_sponsorship_programs
            INNER JOIN ngo_client_loc_supported_sponprogs ON sc_lKeyID=clsp_lSponProgID
         WHERE clsp_lLocID=$lLocID
   ...
   //---------------------------------------------------------------------
      $sqlStr =
        "SELECT
         ...
         FROM ngo_lists_sponsorship_programs
            INNER JOIN ngo_client_loc_supported_sponprogs ON sc_lKeyID=clsp_lSponProgID
            INNER JOIN ngo_admin_aco                      ON sc_lACO=aco_lKeyID
         WHERE clsp_lLocID=$lLocID
         ORDER BY sc_strProgram, sc_lKeyID;";
   ...
   //---------------------------------------------------------------------
      $sqlStr =
        "SELECT
         ...
         FROM ngo_client_xfers
            INNER JOIN ngo_client_status_cats AS oldStatCat ON oldStatCat.csc_lKeyID=cx_lOldStatCatID
            INNER JOIN ngo_client_status_cats AS newStatCat ON newStatCat.csc_lKeyID=cx_lNewStatCatID

            INNER JOIN ngo_client_location AS oldLoc ON oldLoc.cl_lKeyID=cx_lOldLocID
            INNER JOIN ngo_client_location AS newLoc ON newLoc.cl_lKeyID=cx_lNewLocID

            INNER JOIN ngo_lists_client_vocab AS oldVoc ON oldVoc.cv_lKeyID=cx_lOldVocID
            INNER JOIN ngo_lists_client_vocab AS newVoc ON newVoc.cv_lKeyID=cx_lNewVocID

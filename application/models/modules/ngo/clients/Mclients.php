...
class Mclients extends CI_Model{
   ...
   //-----------------------------------------------------------------------
      ...
      $sqlStr =
          "INSERT INTO ngo_client_records
           SET $sqlCommon,
      ...
      $sqlStr =
           "SELECT
            ...
            FROM ngo_client_records
               ...
               LEFT JOIN ngo_client_status                      ON csh_lClientID   = cr_lKeyID
               LEFT JOIN ngo_lists_client_status_entries        ON csh_lStatusID   = cst_lKeyID
               LEFT JOIN ngo_client_status_cats AS curStatSCat  ON cst_lClientStatusCatID = curStatSCat.csc_lKeyID
               LEFT JOIN ngo_client_status_cats AS defSCat      ON cr_lStatusCatID        = defSCat.csc_lKeyID

               INNER JOIN users  AS uc ON uc.id=cr_lOriginID
               INNER JOIN users  AS ul ON ul.id=cr_lLastUpdateID

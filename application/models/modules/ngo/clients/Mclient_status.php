   ...
   //---------------------------------------------------------------------
      $sqlStr =
        "SELECT COUNT(*) AS lNumRecs
         FROM ngo_lists_client_status_entries
         WHERE
...
//      }
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
          'INSERT INTO ngo_client_status_cats
   ...
   //---------------------------------------------------------------------
      ...
      if ($bNoStatCat){
         ...
         if ($gbAdmin){
            ...
            $strOutMsg .= 'Please contact your '.APPNAME.' administrator to add one or more client status categories.<br><br>';
         }
      }
      return($bNoStatCat);
   }
...
//      }
   ...
   public function removeCategory($lCatID){
      ...
      $this->db->query("UPDATE ngo_client_status_cats
                        SET csc_bRetired=1,
                           csc_lLastUpdateID=$glUserID
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
           "SELECT
               ...
               csc_strCatName
            FROM ngo_lists_client_status_entries
               INNER JOIN ngo_client_status_cats ON cst_lClientStatusCatID=csc_lKeyID
            WHERE 1 $strWhere "
...
//      }
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
          'INSERT INTO ngo_lists_client_status_entries
...
//      }
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
          'UPDATE ngo_lists_client_status_entries
           SET '.$this->sqlCommonStatEntry().',
...
//      }
   ...
   //---------------------------------------------------------------------
      $glUserID = USERID;
      $this->db->query("UPDATE ngo_lists_client_status_entries
                        SET cst_bRetired=1,
                           cst_lLastUpdateID=$glUserID
   ...
   //---------------------------------------------------------------------
      ...
      if ($bNoStatEntry && $bShowNotice){
         ...
         if ($gbAdmin){
            ...
            $strErrorMsg .= 'Please contact your '.APPNAME.' administrator to add one or more client status entries.<br><br>';
         }
      }
      return($bNoStatEntry);
   }
  ...
  //---------------------------------------------------------------------
      ...
      $sqlStr =
         "SELECT cst_lKeyID, cst_bAllowSponsorship, cst_bShowInDir, cst_strStatus,
          ...
          FROM ngo_tbl_sponsor_cstat_history
              INNER JOIN ngo_tbl_sponsor_cstatus ON csh_lStatusID=cst_lKeyID
      ...
      $sqlStr =
        "SELECT COUNT(tmp_lKeyID) AS lNumStat, cr_lKeyID, cr_strFName,
         ...
         FROM ngo_client_records
            ...
            LEFT JOIN ngo_client_status_cats ON cr_lStatusCatID=csc_lKeyID

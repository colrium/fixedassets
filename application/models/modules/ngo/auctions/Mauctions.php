...
class Mauctions extends CI_Model{
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
        "SELECT
         ...
         FROM ngo_gifts_auctions
            INNER JOIN users AS usersC   ON auc_lOriginID     = usersC.id
            INNER JOIN users AS usersL   ON auc_lLastUpdateID = usersL.id
            INNER JOIN ngo_admin_aco               ON auc_lACOID        = aco_lKeyID
            INNER JOIN ngo_gifts_campaigns         ON auc_lCampaignID   = gc_lKeyID
            INNER JOIN ngo_gifts_accounts          ON gc_lAcctID        = ga_lKeyID
            LEFT  JOIN ngo_gifts_auctions_bidsheets ON abs_lKeyID        = auc_lDefaultBidSheet
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
         'INSERT INTO ngo_gifts_auctions
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
        "UPDATE ngo_gifts_auctions_bidsheets

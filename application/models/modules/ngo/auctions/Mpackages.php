...
class Mpackages extends CI_Model{
   ...
   //---------------------------------------------------------------------
      $sqlStr = "
         ...
         WHERE ap_lAuctionID=? AND NOT ap_bRetired;";
      ...
      $sqlStr =
        "SELECT
         ...
         FROM ngo_gifts_auctions_packages
            ...
            INNER JOIN ngo_admin_aco                ON auc_lACOID       = aco_lKeyID
            INNER JOIN users AS usersC    ON ap_lOriginID     = usersC.id
            INNER JOIN users AS usersL    ON ap_lLastUpdateID = usersL.id
            LEFT  JOIN ngo_gifts                    ON ap_lGiftID       = gi_lKeyID
            LEFT  JOIN ngo_gifts_auctions_bidsheets ON abs_lKeyID       = ap_lBidSheetID
            LEFT  JOIN ngo_donors AS bwPeople ON pe_lKeyID        = ap_lBidWinnerID
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr = '
          INSERT INTO ngo_gifts_auctions_packages
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr = '
          UPDATE ngo_gifts_auctions_packages
          SET '.$this->strSqlCommonPackageAddEdit()."
          WHERE ap_lKeyID=?;";
      $this->db->query($sqlStr, array($lPackageID));
   }
   ...
   //---------------------------------------------------------------------
      $sqlStr =
          'UPDATE ngo_gifts_auctions_packages
           ...
           WHERE ap_lKeyID=?;";
      $this->db->query($sqlStr, array($lDonorID, $lPackageID));
   }
      ...
      $sqlStr =
          "UPDATE ngo_gifts_auctions_packages
           SET
              ap_lGiftID = ?
           WHERE ap_lKeyID=?;";
      $this->db->query($sqlStr, array($lGiftID, $lPackageID));
   }
      ...
      $sqlStr =
          "UPDATE ngo_gifts_auctions_packages
           SET
              ...
              ap_lLastUpdateID = $glUserID
           WHERE ap_lKeyID=?;";
      $this->db->query($sqlStr, $lPackageID);
   }
}

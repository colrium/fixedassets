...
class Mimage_doc extends CI_Model{
   ...
   ------------------------------------------------------------------------------- */
      ...
      if ($bPDF){
         $strImageLink = '<img src="'.DL_IMAGEPATH.'/misc/pdfIcon.png" border="0" title="Open document in new window">';
      }else {
         $strImageLink = '';
      }
      ...
      if ($lNumObjects > 0){
         ...
         for ($idx=0; $idx<$lNumObjects; ++$idx){
            ...
            if ($opts->bAddRadioSelect){
               $strOut .= '<tr><td style="vertical-align: top;">'
                             .'<div class="radio radio-success"><label><input type="radio" name="'.$opts->strRadioFieldName.'" '
                                  .'value="'.$imageDoc->lKeyID.'" '
                                  .($imageDoc->lKeyID == $opts->lMatchID ? 'checked' : '').'> Select</label></div></td></tr>'."\n";
            }
            $strOut .=
                               '<tr>
         ...
         if ($opts->bShowNone){
            ...
            $strOut .= '<td style="'.$strBorder.$strWidth.' vertical-align: top;">'
                          .'<div class="radio radio-success"><label><input type="radio" name="'.$opts->strRadioFieldName.'" '
                               .'value="-1" '
                               .($opts->lMatchID == -1 ? 'checked' : '').'> '.$opts->strShowNoneLabel.'</label></div></td>';
         }
         $strOut .= '</table>';
      }
      return($strOut);

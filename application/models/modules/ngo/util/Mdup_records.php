...
class Mdup_records extends CI_Model{
   ...
   //---------------------------------------------------------------------
      ...
      $query = $this->db->query($sqlStr);
         ...
         // supported programs
      $sqlStr =
        "UPDATE ngo_client_supported_sponprogs
      ...
      foreach ($dupIDs as $dupID){
         ...
         if (!is_null($lDupVolID)){
            if (is_null($lVolID)){
               ...
               $lVolID = $lDupVolID;
               $sqlStr = "UPDATE ngo_volunteers SET vol_lPeopleID=$lGoodID WHERE vol_lKeyID=$lVolID;";
               $query = $this->db->query($sqlStr);
            }else {
   ...
   //
   //---------------------------------------------------------------------
      $sqlStr = "SELECT ngo_vol_lKeyID FROM volunteers WHERE vol_lPeopleID=$lPID LIMIT 0,1;";
      $query = $this->db->query($sqlStr);
      if ($query->num_rows() == 0){
         $lVolID = null;
      }else {

...
// ActionAid CRM!
//
// copyright (c) 2014-2015 by  Collins Riungu
// Nairobi Kenya
//
...
class Mcreports extends CI_Model{
   ...
   //-----------------------------------------------------------------------
      ...
      foreach ($query->result() as $row){
         ...
         $cU->lUserID       = $row->id;
         $cU->strUserName   = $row->username;
         $cU->strFirstName  = $row->first_name;
         $cU->strLastName   = $row->last_name;
         $cU->strSafeNameFL = htmlspecialchars($row->first_name.' '.$row->last_name);
         $cU->bCurrentUser  = $row->id==$glUserID;
         $cU->lRptCnt       = $this->lNumRptsViaUserID($row->id, $cU->bCurrentUser || $gbAdmin);
         ++$idx;
      }
   }
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
        "SELECT
         ...
         FROM ngo_creport_dir
            INNER JOIN users AS usersC ON crd_lOriginID     = usersC.id
            INNER JOIN users AS usersL ON crd_lLastUpdateID = usersL.id
         WHERE
            NOT crd_bRetired
            AND crd_lOriginID=$lUserID
            $strSQLWhereExtra
   ...
   //---------------------------------------------------------------------
         // remove sort terms
      $sqlStr = "DELETE FROM ngo_creport_sort WHERE crst_lReportID=$lReportID";
      $this->db->query($sqlStr);

         // remove search terms
      $sqlStr = "DELETE FROM ngo_creport_search WHERE crs_lReportID=$lReportID";
      $this->db->query($sqlStr);

         // remove reporting fields
      $sqlStr = "DELETE FROM ngo_creport_fields WHERE crf_lReportID=$lReportID";
      $this->db->query($sqlStr);

         // remove the report
      $sqlStr = "DELETE FROM ngo_creport_dir WHERE crd_lKeyID=$lReportID";
      $this->db->query($sqlStr);
   }
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
        "SELECT crf_lKeyID, crf_lReportID, crf_strFieldName, crf_lSortIDX,
         ...
         FROM ngo_ngo_creport_fields
            LEFT JOIN ngo_uf_tables ON pft_lKeyID   = crf_lTableID
            LEFT JOIN ngo_uf_fields ON crf_lFieldID = pff_lKeyID
         WHERE crf_lReportID=$lReportID
...
*/
   ...
   //---------------------------------------------------------------------
      $sqlStr = "SELECT crst_lReportID FROM ngo_creport_sort WHERE crst_lKeyID=$lSortTermID;";
      $query = $this->db->query($sqlStr);
   ...
   //
   //---------------------------------------------------------------------
      $sqlStr = "DELETE FROM ngo_creport_sort WHERE crst_lKeyID=$lSortTermID;";
      $this->db->query($sqlStr);
   }
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
        "SELECT
         ...
         FROM ngo_creport_sort
            INNER JOIN users AS usersC ON crst_lOriginID      = usersC.id
            INNER JOIN users AS usersL ON crst_lLastUpdateID  = usersL.id

            LEFT JOIN ngo_uf_fields ON pff_lKeyID   = crst_lFieldID
            LEFT JOIN ngo_uf_tables ON pff_lTableID = pft_lKeyID
...
// ------------------------------------- */
   ...
   //---------------------------------------------------------------------
      ...
      switch ($enumRptType){
         case CE_CRPT_GIFTS:
               ...
               // Gift table
               //----------------------
            $this->addReportTable($tables, $lTableIDX, $lFieldIDX, 'Gifts', 'ngo_gifts', CL_STID_GIFTS);
            $this->addBaseGiftFields($tables, $lTableIDX);

               //----------------------
               // Donor table
               //----------------------
            $this->addReportTable($tables, $lTableIDX, $lFieldIDX, 'Donor', 'ngo_donors', CL_STID_PEOPLEBIZ);
            $this->addBaseDonorFields($tables, $lTableIDX);
            break;

         case CE_CRPT_PEOPLE:
            $this->addReportTable($tables, $lTableIDX, $lFieldIDX, 'People', 'ngo_donors', CL_STID_PEOPLE);
         ...
         case CE_CRPT_CLIENTS:
            $this->addReportTable($tables, $lTableIDX, $lFieldIDX, 'Client', 'ngo_client_records', CL_STID_CLIENT);
   ...
   //---------------------------------------------------------------------
      ...
      $this->addReportFields($fields, $lFieldIDX, CS_FT_TEXT,          'State',              'pe_strState');
      $this->addReportFields($fields, $lFieldIDX, CS_FT_TEXT,          'Country',            'pe_strCountry');
      $this->addReportFields($fields, $lFieldIDX, CS_FT_TEXT,          'Zip',                'pe_strZip');
...
*/
   ...
   //---------------------------------------------------------------------
      ...
      $this->addReportFields($fields, $lFieldIDX, CS_FT_TEXT,        'State',              'cr_strState');
      $this->addReportFields($fields, $lFieldIDX, CS_FT_TEXT,        'Country',            'cr_strCountry');
      $this->addReportFields($fields, $lFieldIDX, CS_FT_TEXT,        'Zip',                'cr_strZip');
   ...
   //---------------------------------------------------------------------
      ...
      foreach ($fields as $field){
         ...
         $sqlStr =
             "INSERT INTO ngo_creport_fields
              SET
   ...
   //---------------------------------------------------------------------
      $sqlStr =
         "DELETE FROM ngo_creport_fields WHERE crf_lReportID=$lReportID;";
      $this->db->query($sqlStr);
   }
   ...
   //---------------------------------------------------------------------
      $sqlStr =
         "DELETE FROM ngo_creport_fields

   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
           "SELECT
            ...
            FROM ngo_deposit_log
               INNER JOIN users AS usersC ON dl_lOriginID     = usersC.id
               INNER JOIN users AS usersL ON dl_lLastUpdateID = usersL.id
               INNER JOIN ngo_admin_aco             ON dl_lACOID        = aco_lKeyID
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
        "SELECT
         ...
         FROM ngo_gifts
             INNER JOIN ngo_lists_generic AS listPayType ON gi_lPaymentType = listPayType.lgen_lKeyID
         WHERE
            NOT gi_bRetired
            AND gi_lDepositLogID=$lDepositID
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
            "SELECT 
               ...
               CONCAT(usersC.first_name, ' ', usersC.last_name) AS `Created By`,
               CONCAT(usersL.first_name, ' ', usersL.last_name) AS `Last Updated By`,
             ...
             FROM $strTable
               INNER JOIN  ngo_deposit_log          ON dl_lKeyID        = tmp_lDepositID
               INNER JOIN users AS usersC ON dl_lOriginID     = usersC.id
               INNER JOIN users AS usersL ON dl_lLastUpdateID = usersL.id
               INNER JOIN ngo_admin_aco             ON dl_lACOID        = aco_lKeyID
            WHERE NOT dl_bRetired

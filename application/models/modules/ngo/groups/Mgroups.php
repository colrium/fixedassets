...
class Mgroups extends CI_Model{
   ...
   //-----------------------------------------------------------------------
      ...
      $sqlStr =
        "SELECT gp_lKeyID, gp_strGroupName, gp_strNotes,
         ...
         FROM ngo_groups_child
            INNER JOIN ngo_groups_parent ON gc_lGroupID=gp_lKeyID
   ...
   //---------------------------------------------------------------------
      $strOut =
         '<select class="form-control2 chosen" name="'.$strDDLName.'">'."\n";
      if ($bAddBlank){
   ...
   //-----------------------------------------------------------------------
      ...
      $sqlStr =
        "DELETE ngo_groups_child.*
            FROM ngo_groups_child
            INNER JOIN ngo_tbl_group_parent ON gp_lKeyID=gc_lGroupID
   ...
   //---------------------------------------------------------------------
         // children first
      $sqlStr = "DELETE FROM ngo_groups_child WHERE gc_lGroupID=$this->lGroupID;";
      $query = $this->db->query($sqlStr);

      $sqlStr = "DELETE FROM ngo_groups_parent WHERE gp_lKeyID=$this->lGroupID;";
      $query = $this->db->query($sqlStr);
   }
      ...
      switch ($enumGroupType){
         case CENUM_CONTEXT_BIZ:
            $sqlStr =
              "SELECT COUNT(*) AS lNumRecs
               FROM ngo_groups_child
                  INNER JOIN ngo_donors ON pe_lKeyID=gc_lForeignID
         ...
         case CENUM_CONTEXT_CLIENT:
            $sqlStr =
              "SELECT COUNT(*) AS lNumRecs
               FROM ngo_groups_child
                  INNER JOIN ngo_client_records  ON cr_lKeyID      = gc_lForeignID
         ...
         case CENUM_CONTEXT_SPONSORSHIP:
            $sqlStr =
              "SELECT COUNT(*) AS lNumRecs
               FROM ngo_groups_child
                  INNER JOIN ngo_sponsor      ON sp_lKeyID     = gc_lForeignID
         ...
         case CENUM_CONTEXT_STAFF_TS_PROJECTS:
            $sqlStr =
              "SELECT COUNT(*) AS lNumRecs
               FROM ngo_groups_child
                  INNER JOIN ngo_staff_timesheets  ON ts_lKeyID = gc_lForeignID
               WHERE gc_lGroupID=$lGroupID
                  AND NOT ts_bRetired;";
            break;
         ...
         case CENUM_CONTEXT_USER:
            $sqlStr =
              "SELECT COUNT(*) AS lNumRecs
               FROM ngo_groups_child
                  INNER JOIN users      ON id     = gc_lForeignID
               WHERE gc_lGroupID=$lGroupID
                  AND NOT active;";
            break;
         ...
         case CENUM_CONTEXT_VOLUNTEER:
            $sqlStr =
              "SELECT COUNT(*) AS lNumRecs
               FROM ngo_groups_child
                  INNER JOIN ngo_volunteers   ON vol_lKeyID=gc_lForeignID
      ...
      switch ($enumGroupType){
         ...
         case CENUM_CONTEXT_USER:
            ...
            $sqlStr =
              "SELECT
                  gc_lForeignID AS lKeyID, gc_dteAdded,
                  CONCAT(last_name, ', ', first_name) AS strName,
                  CONCAT(us_strAddr1, '\\n',
                         us_strAddr2, '\\n',
                         us_strCity,  '\\n',
               ...
               WHERE $strGroupWhere
                  AND NOT active
               ORDER BY last_name, first_name, gc_lForeignID;";
            break;
            ...
            case CENUM_CONTEXT_PEOPLE:
               $sqlStr =
                 'SELECT DISTINCT pe_lKeyID
                  FROM ngo_donors
                     INNER JOIN ngo_groups_child  ON pe_lKeyID=gc_lForeignID
                     INNER JOIN ngo_groups_parent ON gc_lGroupID=gp_lKeyID
            ...
            case CENUM_CONTEXT_SPONSORSHIP:
               $sqlStr =
                 'SELECT DISTINCT sp_lKeyID
                  FROM ngo_sponsor
                     INNER JOIN ngo_groups_child  ON sp_lKeyID   = gc_lForeignID
                     INNER JOIN ngo_groups_parent ON gc_lGroupID = gp_lKeyID
            ...
            case CENUM_CONTEXT_VOLUNTEER:
               $sqlStr =
                 'SELECT DISTINCT vol_lKeyID
                  FROM ngo_volunteers
                     INNER JOIN ngo_groups_child  ON vol_lKeyID  = gc_lForeignID
                     INNER JOIN ngo_groups_parent ON gc_lGroupID = gp_lKeyID
                     INNER JOIN ngo_donors  ON pe_lKeyID   = vol_lPeopleID
                  WHERE gp_enumGroupType='.strPrepStr($enumContext).'
                     AND gp_lKeyID IN ('.$strGroupList.')
               ...
               $sqlStr =
                 'SELECT DISTINCT cr_lKeyID
                  FROM ngo_client_records
                     INNER JOIN ngo_groups_child  ON cr_lKeyID  = gc_lForeignID
                     INNER JOIN ngo_groups_parent ON gc_lGroupID = gp_lKeyID
                  WHERE gp_enumGroupType='.strPrepStr($enumContext).'
                     AND gp_lKeyID IN ('.$strGroupList.')
                     AND NOT cr_bRetired '
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
           'CREATE TEMPORARY TABLE ngo_tmpGroupMatch (
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
         'SELECT '.strExportFields_Sponsor().'
          FROM ngo_tmpGroupMatch
             INNER JOIN ngo_sponsor                    ON sp_lKeyID            = gm_lForeignID
             ...
             LEFT  JOIN ngo_lists_client_vocab         ON cv_lKeyID            = cr_lVocID
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
         'SELECT '.strExportFields_Vol().'
          FROM ngo_tmpGroupMatch
             INNER JOIN ngo_volunteers              ON vol_lKeyID=gm_lForeignID
             INNER JOIN ngo_donors AS peepTab ON peepTab.pe_lKeyID   = vol_lPeopleID
          ORDER BY gm_lKeyID;';
      $query = $this->db->query($sqlStr);
      $rptExport = $this->dbutil->csv_from_result($query);
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
         "SELECT $strFields
          FROM ngo_tmpGroupMatch
             INNER JOIN ngo_donors AS $strTableAlias ON $strTableAlias.pe_lKeyID=gm_lForeignID
          ORDER BY gm_lKeyID;";
      $query = $this->db->query($sqlStr);
      $rptExport = $this->dbutil->csv_from_result($query);
      if ($this->config->item('dl_addExportRptInfo')){
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
         'SELECT '.strExportFields_Client().'
          FROM ngo_tmpGroupMatch
             INNER JOIN ngo_client_records              ON cr_lKeyID              = gm_lForeignID
             INNER JOIN ngo_client_location             ON cr_lLocationID         = cl_lKeyID
   ...
   //---------------------------------------------------------------------
      ...
      foreach ($query->result() as $row){
         ...
         $strOut .=
              '<tr>
                  <td class="table enpRpt" style="width: 65px;">'
   ...
   //---------------------------------------------------------------------
      switch ($enumContext){
         case CENUM_CONTEXT_PEOPLE:
            $strIDLabel = 'PeopleID';
            $strInner   = ' INNER JOIN ngo_donors ON pe_lKeyID=gm_lForeignID ';
         ...
         case CENUM_CONTEXT_VOLUNTEER:
            ...
            $strInner   = ' INNER JOIN ngo_volunteers   ON vol_lKeyID    = gm_lForeignID '
         ...
         case CENUM_CONTEXT_SPONSORSHIP:
            ...
            $strInner   = ' INNER JOIN ngo_sponsor   ON sp_lKeyID        = gm_lForeignID '
                         .' INNER JOIN ngo_donors ON sp_lForeignID = pe_lKeyID ';
   ...
   //---------------------------------------------------------------------
      $strOut =
         '<select class="form-control" name="'.$strSelectName.($bMulti ? '[]' : '').'" '

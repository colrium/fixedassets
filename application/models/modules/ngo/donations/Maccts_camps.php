...
// ActionAid CRM!
//
// copyright (c) 2011 by  Collins Riungu
// Nairobi Kenya
//
...
class Maccts_camps extends CI_Model{
   ...
   //-----------------------------------------------------------------------
      ...
      $sqlStr =
           'SELECT
               ...
               ga_bProtected, ga_bSponsorship, ga_strGoal,
      ...
      if ($numRows==0) {
         ...
         foreach ($query->result() as $row){
            ...
            $this->accounts[$idx]->strGoal       = (int)$row->ga_strGoal;
   ...
   //-----------------------------------------------------------------------
      $glUserID = USERID;
      $sqlStr =
         'INSERT INTO ngo_gifts_accounts
   ...
   //-----------------------------------------------------------------------
      ...
      return('
           ...
           ga_strGoal     = '.strPrepStr($this->accounts[0]->strGoal).",
   ...
   //-----------------------------------------------------------------------
      $glUserID = USERID;
      $sqlStr =
         'INSERT INTO ngo_gifts_campaigns
   ...
   //-----------------------------------------------------------------------
      ...
      $sqlStr =
         "INSERT INTO ngo_gifts_campaigns
          SET
             gc_lAcctID       = ".$this->db->escape($lAcctID).",
   ...
   //-----------------------------------------------------------------------
      ...
      $sqlStr =
        'SELECT
         ...
         FROM ngo_gifts_campaigns
            LEFT JOIN ngo_gifts_accounts ON gc_lAcctID=ga_lKeyID
         WHERE 1 '
   ...
   //-----------------------------------------------------------------------
      ...
      $sqlStr =
           "UPDATE ngo_gifts_campaigns
               SET
               gc_lAcctID=".$this->db->escape($lNewAID).",
               gc_lLastUpdate=$glUserID
            WHERE gc_lKeyID=".$this->db->escape($lCampID).";";
      $query = $this->db->query($sqlStr);
   }
   ...
   public function accountCampaignsList($id){
   ...
   public function campTotal($campID){
    $sqlStr =
        "SELECT 
        SUM(gi_curAmnt) AS campaignTotal
         FROM ngo_gifts
         WHERE gi_lCampID=$campID
         AND gi_lACOID=31
         AND NOT gi_bRetired";
   ...
   public function accountGoal($id){
      $sqlStr =
        "SELECT 
        ga_strGoal AS accountGoal
         FROM ngo_gifts_accounts
         WHERE ga_lKeyID=$id";

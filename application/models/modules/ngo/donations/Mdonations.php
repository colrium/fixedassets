...
// ActionAid CRM!
//
// copyright (c) 2011-2014 by  Collins Riungu
// Nairobi Kenya
//
...
class Mdonations extends CI_Model{
   ...
   //---------------------------------------------------------------------
      $sqlStr =
        "SELECT pe_bBiz
         ...
         WHERE gi_lKeyID=".strPrepStr($lGiftID).";";
   ...
   //-----------------------------------------------------------------------
      if (is_array($lGiftID)){
         ...
         $this->sqlExtraWhere = " AND gi_lKeyID=".strPrepStr($lGiftID)." ";
      }
      if ($strSort==''){
      ...
      return(
        "SELECT
         ...
         FROM ngo_gifts
            ...
            INNER JOIN ngo_gifts_accounts  ON gc_lAcctID = ga_lKeyID
            INNER JOIN ngo_donors    ON pe_lKeyID  = gi_lForeignID
            INNER JOIN ngo_admin_aco       ON gi_lACOID  = aco_lKeyID

            INNER JOIN users AS usersC ON gi_lOriginID    = usersC.id
            INNER JOIN users AS usersL ON gi_lLastUpdateID= usersL.id

            LEFT  JOIN ngo_deposit_log                       ON gi_lDepositLogID = dl_lKeyID
            ...
            LEFT  JOIN ngo_lists_generic AS listMajorGiftCat ON gi_lMajorGiftCat = listMajorGiftCat.lgen_lKeyID
...
//            $gift->gi_dteDonation    = (int)$row->dteDonation;
   ...
   //---------------------------------------------------------------------
      ...
      switch ($this->cumulativeOpts->enumCumulativeSource){
         ...
         case 'biz':
            ...
            $strWhereFID = " AND (gi_lForeignID=".strPrepStr($lForeignID).") ";
            $strInner    = '';
            break;
         ...
         case 'account':
            ...
            $strInner    = ' INNER JOIN ngo_gifts_campaigns ON gi_lCampID=gc_lKeyID ';
            $strWhereFID = " AND (gc_lAcctID=".strPrepStr($this->lAcctID).") ";
            break;
         ...
         case 'campaign':
            ...
            $strInner    = '';
            $strWhereFID = " AND (gi_lCampID=".strPrepStr($this->lCampID).") ";
            break;

         case 'sponsorPayments':
            $lForeignID = $this->lPeopleID;
            if (is_null($lForeignID)) screamForHelp('FOREIGN KEY NOT SET<br>error on <b>line:</b> '.__LINE__.'<br><b>file: </b>'.__FILE__.'<br><b>function: </b>'.__FUNCTION__);
            $strWhereFID = " AND (gi_lForeignID=".strPrepStr($lForeignID).") AND (gi_lSponsorID IS NOT NULL) ";
            $strInner    = '';
            break;
      ...
      $sqlStr =
           "SELECT SUM(gi_curAmnt) AS curTotGift, COUNT(*) AS lNumRecs
            ...
            WHERE NOT gi_bRetired
               ...
               $strWhereMon
               $strWhereFID
               AND gi_lACOID=".strPrepStr($lACOID).";";
   ...
   //---------------------------------------------------------------------
      ...
      if (!$bBiz){
         $sqlStr =
           "SELECT SUM(gi_curAmnt) AS curSumSoft, COUNT(*) AS lNumRecs
            FROM ngo_gifts
               INNER JOIN ngo_donor_relationships ON pr_lPerson_A_ID=gi_lForeignID
            WHERE pr_lPerson_B_ID=".strPrepStr($this->lPeopleID)."
      ...
      if (!$bBiz){
         $sqlStr =
           "SELECT SUM(gi_curAmnt) AS curSumSoft, COUNT(*) AS lNumRecs
            FROM ngo_gifts
               INNER JOIN ngo_biz_contacts ON bc_lBizID=gi_lForeignID
            WHERE bc_lContactID=".strPrepStr($this->lPeopleID)."
               AND bc_bSoftCash
               AND gi_lACOID=".strPrepStr($lACOID)."
               $strWhereDate
               $strWhereMon
      ...
      if ($bBiz){
         $sqlStr =
           "SELECT SUM(gi_curAmnt) AS curSumSoft, COUNT(*) AS lNumRecs
            FROM ngo_gifts
               INNER JOIN ngo_biz_contacts ON bc_lContactID=gi_lForeignID
            WHERE bc_lBizID=".strPrepStr($this->lPeopleID)."
               AND bc_bSoftCash
               AND gi_lACOID=".strPrepStr($lACOID)."
               $strWhereDate
               $strWhereMon
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
         "INSERT INTO $strTableName (tgi_lGiftID, tgi_lPID)
             ...
             FROM ngo_gifts
             WHERE gi_lForeignID=".strPrepStr($lPID)."
                AND gi_lACOID=".strPrepStr($lACOID)."
                AND NOT gi_bRetired;";
      ...
      $sqlStr =
         "INSERT INTO $strTableName (tgi_lGiftID, tgi_lPID)
             ...
             FROM ngo_gifts
                INNER JOIN ngo_donor_relationships ON pr_lPerson_A_ID=gi_lForeignID
             WHERE pr_lPerson_B_ID=".strPrepStr($lPID)."
                AND gi_lACOID=".strPrepStr($lACOID)."
                AND pr_bSoftDonations
                AND NOT gi_bRetired;";
      $this->db->query($sqlStr);
      ...
      if ($bBiz){
         $sqlStr =
            "INSERT INTO $strTableName (tgi_lGiftID, tgi_lPID)
               ...
               FROM ngo_gifts
               INNER JOIN ngo_biz_contacts ON bc_lContactID=gi_lForeignID
               WHERE bc_lBizID=".strPrepStr($lPID)."
                  AND bc_bSoftCash
                  AND gi_lACOID=".strPrepStr($lACOID)."
                  AND NOT bc_bRetired
                  AND NOT gi_bRetired;";
         ...
         $sqlStr =
            "INSERT INTO $strTableName (tgi_lGiftID, tgi_lPID)
               ...
               FROM ngo_gifts
                  INNER JOIN ngo_biz_contacts ON bc_lBizID=gi_lForeignID
               WHERE bc_lContactID=".strPrepStr($lPID)."
                  AND bc_bSoftCash
                  AND gi_lACOID=".strPrepStr($lACOID)."
                  AND NOT bc_bRetired
                  AND NOT gi_bRetired;";
      ...
      $sqlStr =
        "SELECT
         ...
         FROM $strTableName
            INNER JOIN ngo_gifts           ON gi_lKeyID  = tgi_lGiftID
            INNER JOIN ngo_gifts_campaigns ON gi_lCampID = gc_lKeyID
            INNER JOIN ngo_gifts_accounts  ON gc_lAcctID = ga_lKeyID
            INNER JOIN ngo_donors    ON pe_lKeyID  = gi_lForeignID
            LEFT  JOIN ngo_lists_generic   ON gi_lGIK_ID = lgen_lKeyID
   ...
   //---------------------------------------------------------------------
      $sqlStr =
         "SELECT gi_lKeyID FROM ngo_gifts WHERE gi_lForeignID=".strPrepStr($lPID)." AND NOT gi_bRetired;";
...
//            $row = mysql_fetch_array($result);
   ...
   //---------------------------------------------------------------------
      $GIDs = array();
      $sqlStr =
         "SELECT gi_lKeyID FROM ngo_gifts WHERE gi_lForeignID=".strPrepStr($lFID)." AND NOT gi_bRetired;";
      $query = $this->db->query($sqlStr);
   ...
   public function loadGiftListViaACO(&$lNumGifts,   &$gifts,  $lACOID, $enumListType, $strSort){
   //---------------------------------------------------------------------
   // $enumSortType - date, giftID, name, amount, acctCamp
   //---------------------------------------------------------------------
      switch ($enumListType){
         case 'viaAccounts':
            $sqlWhere = " AND gc_lAcctID = ".strPrepStr($this->lListsAcctID)." ";
            break;
         default:
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
           "SELECT ghml_lKeyID
            ...
            WHERE ghml_lHonMemID=".strPrepStr($lHMID)." AND NOT gi_bRetired
            $strLimit;";
      $query = $this->db->query($sqlStr);
      ...
      $sqlStr =
           'SELECT '.strExportFields_Gifts()."
            FROM ngo_gifts_hon_mem_links
               ...
               INNER JOIN ngo_gifts_campaigns             ON gi_lCampID             = gc_lKeyID
               INNER JOIN ngo_gifts_accounts              ON gc_lAcctID             = ga_lKeyID
               INNER JOIN ngo_donors   AS donor     ON donor.pe_lKeyID        = gi_lForeignID
               INNER JOIN ngo_admin_aco                   ON gi_lACOID              = aco_lKeyID

               LEFT  JOIN ngo_lists_generic AS payCat     ON payCat.lgen_lKeyID     = gi_lPaymentType
               LEFT  JOIN ngo_lists_generic AS giftCat    ON giftCat.lgen_lKeyID    = gi_lMajorGiftCat
               LEFT  JOIN ngo_lists_generic AS giftAttrib ON giftAttrib.lgen_lKeyID = gi_lAttributedTo
               LEFT  JOIN ngo_sponsor                     ON sp_lKeyID              = gi_lSponsorID
               LEFT  JOIN ngo_donors   AS spon      ON spon.pe_lKeyID         = sp_lForeignID
               LEFT  JOIN ngo_lists_generic  AS gik       ON gi_lGIK_ID             = gik.lgen_lKeyID
               LEFT  JOIN users AS ackUser      ON gi_lAckByID            = ackUser.id
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
           "SELECT
               ...
               gi_bAck, gi_lAckByID, first_name, last_name,
               gi_dteAck,
            ...
            FROM ngo_gifts_hon_mem_links
               ...
               INNER JOIN ngo_admin_aco       ON gi_lACOID  = aco_lKeyID
               LEFT  JOIN ngo_lists_generic   ON gi_lGIK_ID = lgen_lKeyID
               LEFT  JOIN users     ON gi_lAckByID= id
            WHERE ghml_lHonMemID=".strPrepStr($lHMID)."
               AND NOT gi_bRetired
               AND NOT pe_bRetired
         ...
         case 'mem':
            $sqlStr =
                 'SELECT gi_lKeyID
                  FROM ngo_gifts
                     INNER JOIN ngo_donors        ON pe_lKeyID      = gi_lForeignID
                     INNER JOIN ngo_gifts_hon_mem_links ON gi_lKeyID      = ghml_lGiftID
                     INNER JOIN ngo_lists_hon_mem       ON ghml_lHonMemID = ghm_lKeyID
                  WHERE NOT pe_bRetired '.$this->strRptAckWhere($dteStart, $dteEnd, $bFindNonAck, $bIncludeSpon, $lACO, $enumAckType).'
   ...
   //---------------------------------------------------------------------
      ...
      if ($bHon){
         ...
         $strExtraJoin = ' LEFT JOIN ngo_donors AS mailCon ON mailCon.pe_lKeyID = ghm_lMailContactID '."\n";
         $strExtraSel  = '
      ...
      $sqlStr =
           "SELECT
               ...
               ghml_bAck, ghml_lAckByID, first_name, last_name,
               ghml_dteAck,
            ...
            FROM ngo_gifts
               ...
               INNER JOIN ngo_gifts_hon_mem_links    ON gi_lKeyID        = ghml_lGiftID
               INNER JOIN ngo_lists_hon_mem          ON ghml_lHonMemID   = ghm_lKeyID
               INNER JOIN ngo_donors AS honMem ON honMem.pe_lKeyID = ghm_lFID
               LEFT  JOIN ngo_lists_generic          ON gi_lGIK_ID       = lgen_lKeyID
               LEFT  JOIN users            ON ghml_lAckByID    = id
               $strExtraJoin
   ...
   //---------------------------------------------------------------------
      ...
      switch ($enumAckType){
         ...
         case 'mem':
            $sqlStr =
                 "UPDATE ngo_gifts_hon_mem_links
   ...
   //---------------------------------------------------------------------
      ...
      if ($bGroupViaDonor){
         $sqlStr = '
            ...
            FROM ngo_gifts '.$this->strRptTimeFrameInner($sRpt).'
            WHERE NOT pe_bRetired '.$this->strRptTimeFrameWhere($sRpt)
               .$this->strRptTimeFrameGroup($sRpt).'
               ORDER BY pe_lKeyID;';
      ...
      if ($bAggregateDonor){
         if ($sRpt->bAggregateDonor){
            ...
            if ($bShowAccounts || $bShowCamps){
               $strInner = '
                  INNER JOIN ngo_gifts_campaigns ON gi_lCampID  = gc_lKeyID
                  INNER JOIN ngo_gifts_accounts  ON gc_lAcctID  = ga_lKeyID '.$strInner;
            }
         }
      }
      return($strInner);

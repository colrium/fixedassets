   ...
   //-----------------------------------------------------------------
      ...
      switch ($enumLoadType){
         ...
         case 'via GiftID':
            ...
            $strInner   = ' INNER JOIN ngo_gifts_hon_mem_links ON ghml_lHonMemID=ghm_lKeyID ';
//            $sqlSelect  = ', ghml_lKeyID, ghml_bAck, UNIX_TIMESTAMP(ghml_dteAck) AS dteAck, ghml_lAckByID ';
      ...
      $sqlStr =
        "SELECT
         ...
         FROM ngo_lists_hon_mem
            INNER JOIN ngo_donors AS tblHM ON tblHM.pe_lKeyID=ghm_lFID
            $strInner
            LEFT  JOIN ngo_donors AS tblMC ON tblMC.pe_lKeyID=ghm_lMailContactID
   ...
   //-----------------------------------------------------------------
      ...
      $sqlStr =
        "INSERT INTO ngo_lists_hon_mem
         SET
            ghm_lFID           = ".$this->db->escape($lPID).",
      ...
      $sqlStr =
        "SELECT COUNT(*) AS lNumRecs
         FROM ngo_gifts_hon_mem_links
            INNER JOIN ngo_gifts ON ghml_lGiftID=gi_lKeyID
      ...
      $sqlStr =
        'UPDATE ngo_lists_hon_mem
         SET
            ...
            ghm_lLastUpdateID=$glUserID
         WHERE ghm_lKeyID=".strPrepStr($lHMID).";";
      $query = $this->db->query($sqlStr);
   }
   ...
   //-----------------------------------------------------------------------
      $sqlStr =
        "INSERT INTO ngo_gifts_hon_mem_links
         SET
            ghml_lGiftID   = ".$this->db->escape($lGID).",
            ghml_lHonMemID = ".$this->db->escape($lHMID).",
            ghml_strNote   = '',
            ghml_bAck      = 0,
            ghml_dteAck    = NULL,
            ghml_lAckByID  = NULL
   ...
   //-----------------------------------------------------------------------
      $sqlStr =
        "DELETE FROM ngo_gifts_hon_mem_links
         WHERE ghml_lKeyID=".strPrepStr($lHMLinkID).";";
      $this->db->query($sqlStr);
   }
   ...
   //-----------------------------------------------------------------------
      $sqlStr =
         "SELECT COUNT(*) AS lNumRecs
          ...
          WHERE ghml_lGiftID=".strPrepStr($lGID)." AND ".($bHon ? '' : ' NOT ').' ghm_bHon;';

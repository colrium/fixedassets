...
class Mpledges extends CI_Model{
   ...
   //
   //---------------------------------------------------------------------
      $this->sqlExtraWhere = " AND gp_lKeyID = ".strPrepStr($lPledgeID)." ";
      $this->loadPledges();
   }
   ...
   //
   //---------------------------------------------------------------------
      $this->sqlExtraWhere = " AND gp_lForeignID = ".strPrepStr($lFID)." ";
      $this->loadPledges();
   }
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
        "SELECT
         ...
         FROM ngo_gifts_pledges
            INNER JOIN ngo_gifts_campaigns ON gp_lCampID = gc_lKeyID
            INNER JOIN ngo_gifts_accounts  ON gc_lAcctID = ga_lKeyID
            INNER JOIN ngo_donors    ON pe_lKeyID  = gp_lForeignID
            INNER JOIN ngo_admin_aco       ON gp_lACOID  = aco_lKeyID

            INNER JOIN users AS usersC ON gp_lOriginID    = usersC.id
            INNER JOIN users AS usersL ON gp_lLastUpdateID= usersL.id

            LEFT  JOIN ngo_lists_generic AS listAttrib       ON gp_lAttributedTo = listAttrib.lgen_lKeyID
...
//      $strOut .= $this->strAddPFreqDDL($enumMatch, , 'other'
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
           'INSERT INTO ngo_gifts_pledges
            SET '.$this->sqlCommonAddUpdate().",
               gp_lForeignID = ".strPrepStr($pledge->lForeignID).",
               gp_bRetired   = 0,
               gp_lOriginID  = ".strPrepStr($glUserID).",
               gp_dteOrigin  = NOW();";
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
           'UPDATE ngo_gifts_pledges
            SET '.$this->sqlCommonAddUpdate()."
            WHERE gp_lKeyID=".strPrepStr($lPledgeID).";";
...
// ------------------------------------- */
      ...
      $sqlStr =
           "SELECT SUM(gi_curAmnt) AS curSum
            ...
            WHERE NOT gi_bRetired
               AND gi_lPledgeID=".strPrepStr($lPledgeID)."
   ...
   // pledge payments.
   //---------------------------------------------------------------------
      $cgifts->sqlExtraWhere = " AND gi_lPledgeID=$lPledgeID AND gi_lACOID=".strPrepStr($lACOID)." ";
   ...
   //---------------------------------------------------------------------
      $sqlStr =
        "SELECT COUNT(*) AS lNumRecs
         FROM ngo_gifts_pledges
         WHERE gp_lForeignID=".strPrepStr($lFID)."
            AND NOT gp_bRetired;";
      $query = $this->db->query($sqlStr);

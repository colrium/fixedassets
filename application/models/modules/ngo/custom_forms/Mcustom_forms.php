...
// ActionAid CRM!
//
// copyright (c) 2013-2014 by  Collins Riungu
// Nairobi Kenya
//
...
class Mcustom_forms extends CI_Model{
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr = "
         ...
         FROM ngo_custom_forms
            INNER JOIN users   AS uc ON uc.id=cf_lOriginID
            INNER JOIN users   AS ul ON ul.id=cf_lLastUpdateID
   ...
   //---------------------------------------------------------------------
      $sqlStr =
        "SELECT cftl_strLabel
         FROM ngo_custom_form_table_labels
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
           "SELECT DISTINCT
            ...
            FROM ngo_custom_form_uf
               INNER JOIN ngo_uf_tables ON cfuf_lTableID=pft_lKeyID
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
           "SELECT
            ...
            FROM ngo_custom_form_uf
               INNER JOIN ngo_uf_fields ON cfuf_lFieldID=pff_lKeyID
   ...
   //---------------------------------------------------------------------
      ...
      $sqlStr =
        "SELECT cfl_lKeyID, cfl_lCFormID, cfl_lForeignID, cfl_lOriginID,
            ...
            uc.first_name AS strUCFName, uc.last_name AS strUCLName
         FROM ngo_custom_form_log
            INNER JOIN users AS uc ON uc.id = cfl_lOriginID

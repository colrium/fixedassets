...
class Barcode{
     ...
     * @public
     */
    public function barcodehtml($code, $type, $widthFactor = 2, $totalHeight = 30, $color = 'black'){
    ...
    public function barcodejpg($code, $type, $widthFactor = 2, $totalHeight = 30, $color = array(0, 0, 0)) {
        $barcodeData = $this->getBarcodeData($code, $type);
     ...
     * @public
     */
    public function barcodepng($code, $type, $widthFactor = 2, $totalHeight = 30, $color = array(0, 0, 0)){
        $barcodeData = $this->getBarcodeData($code, $type);
     ...
     * @public
     */
    public function barcodesvg($code, $type, $widthFactor = 2, $totalHeight = 30, $color = 'black')
    {
        $barcodeData = $this->getBarcodeData($code, $type);
    ...
    protected function barcode_code39($code, $extended = false, $checksum = false)
    {
        ...
        for ($i = 0; $i < $clen; ++$i) {
            $char = $code{$i};
            if ( ! isset($chr[$char])) {
                invalid_character_error('Char ' . $char . ' is unsupported');
            }
            for ($j = 0; $j < 9; ++$j) {
                if (($j % 2) == 0) {
                    $t = true; // bar
    ...
    protected function encode_code39_ext($code)
    {
        ...
        for ($i = 0; $i < $clen; ++$i) {
            if (ord($code{$i}) > 127) {
                invalid_character_error('Only supports till char 127');
            }
    ...
    protected function barcode_msi($code, $checksum = false)
    {
        ...
        for ($i = 0; $i < $clen; ++$i) {
            $digit = $code{$i};
            if ( ! isset($chr[$digit])) {
                invalid_character_error('Char ' . $digit . ' is unsupported');
            }
            $seq .= $chr[$digit];
        }
    ...
    protected function barcode_c128($code, $type = '')
    {
        ...
        switch (strtoupper($type)) {
            ...
            case 'C': { // MODE C
                ...
                if (($len % 2) != 0) {
                    invalidlength_exception('Length must be even');
                }
                for ($i = 0; $i < $len; $i += 2) {
        ...
        if ($upce) {
            ...
            for ($i = $half_len; $i < $len; ++$i) {
                if ( ! isset($codes['C'][$code{$i}])) {
                    invalid_character_error('Char ' . $code{$i} . ' not allowed');
                }
                $seq .= $codes['C'][$code{$i}];
            }
    ...
    protected function barcode_codabar($code)
    {
        ...
        for ($i = 0; $i < $len; ++$i) {
            if ( ! isset($chr[$code{$i}])) {
                invalid_character_error('Char ' . $code{$i} . ' is unsupported');
            }
            $seq = $chr[$code{$i}];
            for ($j = 0; $j < 8; ++$j) {
                if (($j % 2) == 0) {
    ...
    protected function barcode_imb($code)
    {
        ...
        switch (strlen($routing_code)) {
            ...
            default: {
                barcode_exception('Routing code unknown');
                break;
            }
        }

...
# ======================================================================== #
#
#  Title      [PHP] Uploads controller
#  Author:    Mutugi Riungu
#  Website:   http://www.forttech.co.ke
#  Version:   1
#  Date:      27-Oct-2016
...
class Uploader{
    ...
    private function validate($file = null){
        ...
        if($file == null){
            ...
            if($options['maxSize'] && $file['filesizeinbytes'][0] > $options['maxSize']) $errors[] = $this->error_messages['max_file_size'];
    ...
    private function prepareFiles(){
        ...
        if ($this->options['filelocation']!='database' && $this->options['filelocation']!='filesystem') {
            $this->options['filelocation']='filesystem';
        }

        if ($this->options['filelocation']=='database') {
            $savetodb = TRUE;
        }
        ...
        if($validate){
            ...
            for($i = 0; $i < count($field['name']); $i++){
                ...
                $metas['type'] = $field['type'][$i];
                $metas['extension'] = $field['Field_Type'] == 'link' && empty($metas['extension']) ? $metas['type'][1] : $metas['extension'];
                $metas['oldname'] = $field['name'][$i];
                $metas['filesizeinbytes'] = $field['size'][$i];
                $metas['filesize'] = $this->formatSize($metas['filesizeinbytes']);
                $metas['name'] = $this->generateFileName($this->options['title'], array('name'=>substr($metas['oldname'], 0, (!empty($metas['extension']) ? -(strlen($metas['extension'])+1) : strlen($metas['oldname']))), 'size'=>$metas['filesizeinbytes'], 'extension'=> $metas['extension']));
                $metas['file_dir'] = $this->options['uploadDir'] . $metas['name'];
                $metas['entity'] = $this->options['entity'];
                $metas['record'] = $this->options['record'];
                $metas['isimage'] = $this->options['isimage'] == FALSE? 0 : 1;
                $metas['ismainimage'] = $this->options['ismainimage'] == FALSE? 0 : 1;
                $metas['date'] = date('Y-m-d');
                $metas['replaced'] = file_exists($metas['file_dir']);
                $metas['gmtinsertdate'] = date('r');
                $metas['location'] = $this->options['filelocation'];
                ...
                if(!$is_file_removed && $this->validate(array_merge($metas, array('index'=>$i, 'tmp'=>$tmp_name))) && $this->uploadFile($tmp_name, $metas['file_dir'])){
                    if($this->options['perms']) @chmod($metas['file_dir'], $this->options['perms']);

                    $custom = $this->_onUpload($metas, $this->field); if($custom && is_array($custom)) $metas = array_merge($custom, $metas);

                    ksort($metas);
                    array_push($successData, $metas);

                    $files[] = $metas['file_dir'];
                    $this->data['data']['metas'][$i] = $metas;
                }
            }

            

            if ($this->options['databaselog'] == TRUE) {
                foreach ($this->data['data']['metas'] as $key => $value) {
                    $this->data['data']['metas'][$key]['databaseid'] = dbaddattachmentlog($value, $savetodb);
                }
            }
            ...
            if($this->data['isSuccess']) {
                $this->data['data']['databaseids'] = $this->_onSuccess($this->data['data']['metas']);
            }
    ...
    private function _onSuccess(){
        $arguments = func_get_args();
        return $this->options['onSuccess'] != null && function_exists($this->options['onSuccess']) ? $this->options['onSuccess'](@$arguments[0]) : null;
    }

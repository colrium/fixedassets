...
class Ciqrcode{
	...
	public function generate($params = array()) {
		...
		if (isset($params['savename'])) {
			...
			return QRcode::png($params['data'], NULL, $level, $size, 2);
		}
	}


	public function generatetext($params = array()) {
		...
		if (isset($params['savename'])) {
			...
			QRcode::text($params['data'], $params['savename'], $level, $size, 2);
			return $params['savename'];
		} else {
			$level = 'L';
			if (isset($params['level']) && in_array($params['level'], array('L','M','Q','H'))) $level = $params['level'];
			...
			QRcode::text($params['data'], NULL, $level, $size, 2);
		}
	}
}
